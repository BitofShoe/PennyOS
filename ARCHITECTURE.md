...
- `lib/penny-semantic-export.js` and `scripts/export-penny-semantic-claims.js`
  - local read-only semantic export artifacts for normalized claims, memory links, domains, predicates, and local semantic IDs without RDF/SPARQL/triplestore dependencies, URI dereferencing, PromptTruth expansion, tool-evidence changes, memory promotion, live model calls, or route changes
...
Semantic identity/provenance follow-through is artifact-first. Candidate-survival archive-unit runs can report `penny-semantic-claim-trace.v1` beside existing trace/link/contract summaries for QA inspection, prompt-time receipts preserve only compact rendered-claim authority labels for archive items that actually rendered, and `npm run export:semantic-claims` can write a local read-only `penny-semantic-export.v1` JSON artifact for inspection. Raw claim graphs, candidate-only claims, dynamic links, static similarity traces, source graph internals, and semantic export files remain outside PromptTruth and outside `toolEvidenceReceipt`.
...
Local read-only semantic export runner for `penny-semantic-export.v1` plain JSON artifacts. It inspects fixture or supplied local JSON claim/link input plus the current domain/predicate registries, and it does not call LM Studio, dereference IDs, add route surfaces, or introduce RDF/SPARQL/triplestore dependencies.
