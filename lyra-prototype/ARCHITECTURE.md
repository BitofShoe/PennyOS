...
- semantic claim contracts are owned by `lib/penny-semantic-claims.js` as pure local assertion helpers. They can normalize, validate, and summarize source-attributed subject/predicate/object claims with authority, support, temporal, and stale-status fields; candidate-only/static claims cannot be canonical, archive claims default advisory, and the helpers do not change runtime retrieval, PromptTruth, `toolEvidenceReceipt`, prompt limits, memory promotion, runtime voice, or graph infrastructure.
