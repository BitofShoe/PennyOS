...
Semantic identity/provenance follow-through is still artifact-first. Candidate-survival archive-unit runs can now report `penny-semantic-claim-trace.v1` beside existing trace/link/contract summaries, but that trace is for QA inspection only and stays out of prompt-time rendered context until a later prompt-rendering guardrail slice explicitly preserves a bounded authority label.
