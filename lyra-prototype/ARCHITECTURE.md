...
Dynamic memory linking is a future helper-owned layer, not active runtime architecture yet. The current law is deliberately small: links can help navigation, retrieval explanation, correction handling, open-loop resurfacing, and reflection planning, but a memory link is never proof that either endpoint is true. Links must not make archive/research/semantic/static candidates canonical, must not promote advisory memories, must not let candidate-only support count as verified support, must not expand PromptTruth or `toolEvidenceReceipt`, must not change runtime voice, and must not trigger a graph DB or universal index migration. The first later code slices should add schema and fixture traces, then correction-link shadows; only conservative current-vs-stale correction scoring can become active behind a gate after tests. Same-project-thread, research-pattern, and open-loop links remain shadow/advisory until separately measured.
