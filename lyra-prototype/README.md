...
Keep `PENNY_ENABLE_BOUNDED_INITIATIVE` off by default until the bounded aliveness compare harness exists and shows that small suggestions improve follow-through without annoyance, overclaim, prompt bloat, or pressure failures. The current initiative fixture and pressure canaries prove guardrail shape, not default adoption.
Keep `PENNY_ENABLE_TURN_STATE_PROMPT` off by default until the bounded aliveness compare harness proves the scaffold improves timing, format, and source caution without prompt bloat, weak-evidence overclaim, privacy leakage, or pressure failures. The current turn-state fixture and focused tests prove shape and retention guardrails, not default adoption.
