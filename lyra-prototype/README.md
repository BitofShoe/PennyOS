...
- Dynamic memory linking is currently at L0 docs/current-law status only. A future memory link may help retrieval, correction handling, open-loop resurfacing, reflection, candidate traces, and pulse prep, but the core rule is fixed: a memory link is a retrieval/navigation hint, not proof that either side is true. Links do not make advisory memory canonical, do not replace explicit memory, do not activate broad project/research/open-loop scoring before shadow QA, do not expand PromptTruth or `toolEvidenceReceipt`, do not change runtime voice, and do not imply a graph DB or universal memory index.
