...
- Session reflection and memory-suggestion work is currently bounded by the review-gated rule: reflection can summarize, suggest, and prepare reviewable updates, but it cannot silently write canonical explicit memory. Any memory suggestion needs support state, sensitivity, `requiresApproval: true`, and `autoPromoted: false` unless a later explicit approval slice routes it through the existing explicit-memory path. Reflection artifacts are not PromptTruth, not `toolEvidenceReceipt`, not hidden chain-of-thought, and not proof that their summaries are true.
