...
- Runtime artifacts now keep prompt-time `promptTruth` separate from sibling `toolEvidenceReceipt`: `promptTruth` covers candidate-vs-rendered memory/research context plus holdback truth, and rendered archive channels may carry compact `renderedClaims[]` authority labels for prompt-visible claim summaries. `toolEvidenceReceipt` covers deterministic-only, provenance-only, prompt-visible raw JSON, auto-verification, and summarized tool-evidence paths without widening PromptTruth. Tool-cost hints and `toolCostSummary` are advisory sibling runtime artifact metadata; they do not change planner behavior or make cost metadata a runtime authority layer.
- Context-pressure and source-sensitive memory QA now have lightweight fixture-only artifacts and helpers that record estimated prompt tokens, selected/rendered memory counts, lane/model identity placeholders, fixture-assumed semantic readiness, nullable latency fields, `not-run` answer drift, source-sensitive retrieval expectations, structured candidate-contract checks, candidate-survival correlation, and separate answer outcome buckets without making long context the default.
...
- Dynamic memory linking now has helper/fixture/QA/compare code, semantic contract metadata, and gated conservative correction scoring via `PENNY_MEMORY_LINK_SCORING=correction-v1`. The core rule is still fixed: a memory link is a retrieval/navigation hint, not proof that either side is true. Structured links can carry local `linkId`, registered `predicateId`, claim endpoints, source authority, support state, and evidence receipts, but those fields do not make advisory memory canonical, do not replace explicit memory, do not activate broad project/research/open-loop scoring, do not expand PromptTruth or `toolEvidenceReceipt`, do not change runtime voice, and do not imply a graph DB or universal memory index.
...
- Semantic predicate contracts now exist as a pure local registry in `lib/penny-semantic-predicates.js`. The registry defines a small typed predicate set plus explicit ranking, receipt, inverse, and memory-sensitivity flags, including the dynamic-link relations used by memory links; unknown predicates fail closed, labels do not decide behavior, and no runtime scoring, PromptTruth, `toolEvidenceReceipt`, memory-promotion, graph, or prompt behavior changes are implied.
- Semantic domain contracts now exist as a pure local registry in `lib/penny-semantic-domains.js`. They make explicit memory, archive, research-ledger, static-candidate, open-loop, tool-evidence, document-extraction, repo-current-law, runtime-artifact, and fixture authority defaults testable; unknown domains fail closed, repo-current-law cannot canonize personal memory, tool evidence remains a sibling receipt domain rather than PromptTruth, and the registry does not change runtime retrieval, ranking, prompt rendering, memory promotion, graph infrastructure, or prompt limits.
- Semantic claim contracts now exist as pure local helpers in `lib/penny-semantic-claims.js`. They normalize, validate, and summarize subject-predicate-object assertions with source, authority, support, temporal, stale-status, and registered authority-domain fields; candidate-only/static claims cannot become canonical, archive claims default advisory, and the helper does not wire raw claims into runtime retrieval, `toolEvidenceReceipt`, memory promotion, graph infrastructure, or prompt limits.
- Semantic source-ID audit now exists as a fixture/local-audit helper in `lib/penny-semantic-source-audit.js` plus `npm run qa:semantic:source-audit`. It checks explicit/archive/ledger/static/prompt-truth/tool-evidence/open-loop/link/candidate/claim surfaces for missing or unstable source IDs and cache/link mismatches, but it remains QA evidence only and does not expand PromptTruth, change `toolEvidenceReceipt`, prove answer quality, dereference URIs, promote memory, or change runtime behavior.
- Candidate-survival archive-unit QA now carries a sibling `penny-semantic-claim-trace.v1` section when candidate traces contain structured claims. It lets reviewers distinguish expected subject/predicate/object claims from unstructured advisory text, wrong-predicate matches, and candidate-only static/semantic claims without putting raw claim graphs into `archiveContext`, PromptTruth, `toolEvidenceReceipt`, ranking, prompt rendering, memory promotion, or prompt-limit policy.
- Prompt rendering authority guardrails now preserve compact rendered-claim authority labels for archive items that actually enter prompt context. `promptTruth.channels.sessionArchive.renderedClaims[]` and `globalArchive.renderedClaims[]` can carry only `renderedClaimId`, `domainId`, `sourceAuthority`, `supportState`, and `temporalScope`; held-back, stale, fixture-only, and candidate-only claims stay out, and raw semantic graphs, dynamic links, static similarity traces, tool evidence, and candidate traces remain outside PromptTruth.
- Local semantic export is available through `npm run export:semantic-claims`, backed by `lib/penny-semantic-export.js` and `scripts/export-penny-semantic-claims.js`. It writes `penny-semantic-export.v1` plain `penny-json` debug artifacts for normalized claims, memory links, domain registry entries, predicate registry entries, and local semantic IDs. The export is read-only and local-only; it does not add RDF/XML, JSON-LD, SPARQL, triplestores, ontology inference, Linked Data publishing, graph DB replacement, URI dereferencing, PromptTruth or `toolEvidenceReceipt` expansion, memory promotion, ranking changes, prompt-limit changes, runtime voice changes, live model calls, route surfaces, or `server.js` changes.
...
- `npm run qa:memory:candidate-survival-fixture` writes the fixture-only candidate-survival schema, retrieval failure taxonomy, and structured semantic candidate-contract checks. It is model-answer-free and does not require LM Studio.
- `npm run qa:memory:candidate-survival` writes the archive-unit candidate-survival artifact against disposable seeded stores, compares `baseline` and gated `hybrid-v1` profile ordering, includes structured semantic candidate-contract checks plus sibling semantic claim traces, and cleans the disposable memory/archive/embedding/book/ledger state. It is model-answer-free and does not require LM Studio.
- `npm run qa:semantic:source-audit` writes a fixture-only source-ID continuity artifact for explicit memory, archive, ledger, static embedding, PromptTruth rendered IDs, tool-evidence receipt sources, open loops, memory links, candidate traces, and semantic claims. It is model-answer-free and does not require LM Studio.
- `npm run export:semantic-claims` writes a local read-only `penny-semantic-export.v1` JSON artifact for semantic claims, dynamic memory links, domains, predicates, and local semantic IDs. It is a debug/export surface only, not PromptTruth, not `toolEvidenceReceipt`, not public Linked Data, and not a memory promotion path.
...
Use `npm run qa:memory:candidate-survival` for the model-answer-free archive-unit candidate survival fixture. It compares baseline archive scoring against the gated `hybrid-v1` profile on the same disposable stores, can be correlated with rendered context pressure, includes sibling semantic claim traces when structured claims are present, and treats candidate survival as retrieval-path evidence rather than answer-quality evidence. Add `-- --shadow-embed-provider=static` or set `PENNY_EMBED_SHADOW_PROVIDER=static` to append a deterministic static embedding comparison for disposable candidate-survival measurement only.
Use `npm run qa:semantic:source-audit` when checking whether semantic/static/link/rendered artifacts still point back to stable source IDs. It reports missing source IDs, static cache/source mismatches, rendered PromptTruth items without source IDs, dynamic-link endpoint gaps, and semantic-claim source/claim ID drift without treating the audit as PromptTruth, tool evidence, answer-quality proof, memory promotion, or runtime authority.
