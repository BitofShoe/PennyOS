...
- Semantic identity/provenance docs describe local implementation contracts, not graph authority. Stable `penny:*` IDs can help claims, links, traces, vector sources, and rendered-context receipts refer to the same local thing, but IDs are opaque, non-dereferenceable, and not evidence by themselves. They do not authorize RDF/JSON-LD/SPARQL infrastructure, URL fetching, memory promotion, PromptTruth expansion, `toolEvidenceReceipt` changes, or prompt-limit increases.
