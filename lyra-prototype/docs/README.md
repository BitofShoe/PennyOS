...
- Bounded initiative docs describe an optional suggest-only response scaffold, not autonomous action. The live bridge remains off by default behind `PENNY_ENABLE_BOUNDED_INITIATIVE`, capped at one suggestion, cooldown-aware, user-dismissible, source/risk-gated, and separate from PromptTruth / `toolEvidenceReceipt`; the future bounded aliveness compare harness is still required before default enablement.
- Ephemeral turn-state docs describe an optional current-turn response-shaping scaffold, not memory, chain-of-thought, psychological profiling, or truth authority. The live bridge remains off by default behind `PENNY_ENABLE_TURN_STATE_PROMPT`, capped by `PENNY_TURN_STATE_MAX_TOKENS`, redacted before runtime-artifact retention, and separate from PromptTruth / `toolEvidenceReceipt`; the future bounded aliveness compare harness is still required before default enablement.
...
| [plans/penny-external-lessons-master-action-plan-2026-04-21.md](./plans/penny-external-lessons-master-action-plan-2026-04-21.md) | April 21 master action plan | Implementation plan | Current | revised slice map, fixture-only cleanup routing, landed-vs-deferred status through the pressure-watch, Gemma-watch, advisory cost, skill, and deterministic-extraction follow-through slices, and next-slice routing | proof that behavior shipped without checking code, tests, and artifacts |
| [penny-tier1-aliveness-plans/README-tier1-bounded-aliveness-plans.md](./penny-tier1-aliveness-plans/README-tier1-bounded-aliveness-plans.md), [penny-tier1-aliveness-plans/02-open-loop-tracker-plan.md](./penny-tier1-aliveness-plans/02-open-loop-tracker-plan.md), [penny-tier1-aliveness-plans/03-bounded-initiative-policy-plan.md](./penny-tier1-aliveness-plans/03-bounded-initiative-policy-plan.md), and [penny-tier1-aliveness-plans/04-ephemeral-turn-state-card-plan.md](./penny-tier1-aliveness-plans/04-ephemeral-turn-state-card-plan.md) | Tier 1 bounded-aliveness plan bundle | Implementation plan | Current planning handoff | the bounded-aliveness handoffs, north-star sentence, global guardrails, open-loop slice history through docs, bounded initiative slice history through docs, and ephemeral turn-state slice history through docs; verify landed behavior against code/tests/artifacts | proof that any other slice shipped, runtime default law, prompt-limit changes, PromptTruth expansion, explicit-memory authority, chain-of-thought access, autonomous action, or unrequested initiative |
