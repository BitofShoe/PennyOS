...
- Runtime artifacts keep `artifact.modelAdvisory.cleanup` and `artifact.modelAdvisory.authorityPressure` as the stable baseline, then add sibling summaries for `cleanupTransform`, `promptComposition`, `promptTruth`, `reasoningPolicy`, `approximatePath`, and `advisoryMerge`.
