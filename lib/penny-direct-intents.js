...
const DIRECT_INTENT_REASON_CODES = Object.freeze({
  ...
  HISTORY_CAPABILITY: 'history_architecture_capability',
});

const { createDirectIntentReplyApi } = require('./penny-direct-intent-replies');
const { resolveHistoryCapabilityIntent } = require('./penny-history-capability');
...
function createDirectIntentApi({
  ...
  function resolveDirectToolIntent(userText = '') {
    ...
    const historyCapabilityIntent = resolveHistoryCapabilityIntent(text);
    if (historyCapabilityIntent) return historyCapabilityIntent;
    ...
    if (explicitUrl && /\b(read|open|summarize|check|inspect|access|visit|fetch|load|what(?:'s| is) on|what does|tell me about)\b/i.test(lower)) {
