...
function validateResearchLedgerStoreSchema(store) {
  if (!store || typeof store !== 'object' || Array.isArray(store)) {
    return 'research ledger must be a JSON object.';
  }
  if (!store.meta || typeof store.meta !== 'object' || Array.isArray(store.meta)) {
    return 'research ledger must contain a `meta` object.';
  }
  if (!store.topics || typeof store.topics !== 'object' || Array.isArray(store.topics)) {
    return 'research ledger `topics` must be an object.';
  }
  return true;
}
...
function createResearchLedgerApi({
  ...
  const durableLedgerStore = createDurableJsonStore({
    fs,
    path,
    filePath: LEDGER_FILE,
    name: 'research ledger',
    buildDefault: defaultResearchLedgerStore,
    validate: validateResearchLedgerStoreSchema,
    normalize: (store = {}) => {
      const topics = {};
      ...
      return {
        meta: {
          schemaVersion: LEDGER_SCHEMA_VERSION,
          updatedAt: trimIso(store?.meta?.updatedAt, ''),
        },
        topics,
      };
    },
    ensureFile: ensureLedgerFile,
  });

  function readLedgerStore() {
    return durableLedgerStore.read();
  }
  ...
  function writeLedgerStore(store = defaultResearchLedgerStore()) {
    ...
    return durableLedgerStore.write(normalized);
  }
