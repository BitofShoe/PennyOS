...
const { createDurableJsonStore } = require('./penny-durable-json');
const {
  createProviderError,
...
const PHRASE_STOPWORDS = new Set([
  ...
  'bro', 'girl', 'girly', 'brat', 'saying', 'sayin', 'think', 'thinking', 'thought', 'thoughts',
  'penny', 'pennyos', 'lol', 'lmao', 'haha', 'hey', 'heey', 'heeeey', 'hello',
]);
...
function buildEmbeddingsStore(embedModel = '', {
  ...
  const normalizedEmbedModel = String(embedModel || '').trim();
  return {
    meta: {
      schemaVersion: EMBEDDINGS_SCHEMA_VERSION,
      embedModel: normalizedEmbedModel,
      requestFormat: embeddingRequestFormatKeyForModel(normalizedEmbedModel),
      updatedAt: '',
      backgroundVectorization: buildDefaultBackgroundVectorizationStatus(backgroundVectorBatchLimit, backgroundVectorsEnabled),
    },
    items: {},
...
function validateArchiveStoreSchema(store) {
  if (!isPlainRecord(store)) return 'archive must be a JSON object.';
  if (!isPlainRecord(store.meta)) return 'archive must contain a `meta` object.';
  if (!isPlainRecord(store.global)) return 'archive must contain a `global` object.';
  if (!isPlainRecord(store.sessions)) return 'archive `sessions` must be an object.';
  for (const key of ['episodes', 'summaries', 'patterns', 'promotionQueue']) {
    if (Object.hasOwn(store.global, key) && !Array.isArray(store.global[key])) {
      return `archive global.${key} must be an array.`;
    }
  }
  return true;
}

function validateEmbeddingsStoreSchema(store) {
  if (!isPlainRecord(store)) return 'embeddings store must be a JSON object.';
  if (!isPlainRecord(store.meta)) return 'embeddings store must contain a `meta` object.';
  if (!isPlainRecord(store.items)) return 'embeddings store `items` must be an object.';
  return true;
}
...
function createMemoryArchiveApi({
  ...
  function embeddingItemMatchesRequestFormat(raw = {}, model = configuredEmbedModel) {
    const expected = embeddingRequestFormatKeyForModel(model || configuredEmbedModel || '');
    const actual = String(raw?.requestFormat || RAW_EMBEDDING_REQUEST_FORMAT).trim() || RAW_EMBEDDING_REQUEST_FORMAT;
  ...
  function embeddingCacheKeyForText(text = '', model = configuredEmbedModel) {
    ...
    const requestFormat = embeddingRequestFormatKeyForModel(normalizedModel);
    const keyParts = [normalizedModel || 'unknown-embedding-model'];
    if (requestFormat !== RAW_EMBEDDING_REQUEST_FORMAT) keyParts.push(requestFormat);
    keyParts.push(normalizedText);
    return stableHash(keyParts.join('\n'));
  }
  ...
  function findEmbeddingCacheEntry(store = {}, text = '', model = configuredEmbedModel) {
    ...
    if (
      ...
      && embeddingItemMatchesConfiguredModel(direct, model)
      && embeddingItemMatchesRequestFormat(direct, model)
    ) {
      return { item: direct, hash, legacyHash, migrated: false };
    }
    const legacy = legacyHash !== hash ? store?.items?.[legacyHash] : null;
    if (
      legacy?.vector?.length
      && embeddingItemMatchesConfiguredModel(legacy, model)
      && embeddingItemMatchesRequestFormat(legacy, model)
    ) {
  ...
  function normalizeEmbeddingsStore(store = {}) {
    ...
    base.meta = {
      ...
      schemaVersion: EMBEDDINGS_SCHEMA_VERSION,
      embedModel: normalizeEmbedModelId(configuredEmbedModel || (parsed.meta && parsed.meta.embedModel) || ''),
      requestFormat: embeddingRequestFormatKeyForModel(configuredEmbedModel || parsed.meta?.embedModel || ''),
    ...
    for (const [key, raw] of Object.entries(items)) {
      ...
      base.items[key] = {
        ...
        requestFormat: String(raw.requestFormat || RAW_EMBEDDING_REQUEST_FORMAT).trim() || RAW_EMBEDDING_REQUEST_FORMAT,
  ...
  const archiveStore = createDurableJsonStore({
    fs,
    path,
    filePath: ARCHIVE_FILE,
    name: 'memory archive',
    buildDefault: () => buildArchiveStore(configuredEmbedModel, {
      backgroundVectorsEnabled: backgroundChatVectorsEnabled,
      backgroundVectorBatchLimit: backgroundChatVectorBatchLimit,
    }),
    validate: validateArchiveStoreSchema,
    normalize: normalizeArchiveStore,
    ensureFile: () => ensureFile(ARCHIVE_FILE, () => buildArchiveStore(configuredEmbedModel, {
      backgroundVectorsEnabled: backgroundChatVectorsEnabled,
      backgroundVectorBatchLimit: backgroundChatVectorBatchLimit,
    })),
  });

  const embeddingsStore = createDurableJsonStore({
    fs,
    path,
    filePath: EMBEDDINGS_FILE,
    name: 'memory embeddings store',
    buildDefault: () => buildEmbeddingsStore(configuredEmbedModel, {
      backgroundVectorsEnabled: backgroundChatVectorsEnabled,
      backgroundVectorBatchLimit: backgroundChatVectorBatchLimit,
    }),
    validate: validateEmbeddingsStoreSchema,
    normalize: normalizeEmbeddingsStore,
    ensureFile: () => ensureFile(EMBEDDINGS_FILE, () => buildEmbeddingsStore(configuredEmbedModel, {
      backgroundVectorsEnabled: backgroundChatVectorsEnabled,
      backgroundVectorBatchLimit: backgroundChatVectorBatchLimit,
    })),
  });
  ...
  function writeArchiveStore(store = {}) {
    return archiveStore.write(store);
  }

  function readEmbeddingsStore() {
    return embeddingsStore.read();
  }
  ...
  function writeEmbeddingsStore(store = {}, { replace = false } = {}) {
    ...
    return embeddingsStore.write(normalized);
  }
  ...
  async function probeEmbeddingAvailability(model = configuredEmbedModel) {
    ...
    try {
      const response = await fetch(`${embeddingBase}/embeddings`, {
        ...
        body: JSON.stringify({
          model,
          input: formatEmbeddingRequestInput('penny semantic memory probe', model),
        }),
      });
      ...
      if (!response.ok) {
        return {
          ok: false,
          reachable: true,
          error: toPublicProviderError(createProviderError({
            code: 'provider_upstream_error',
            provider: LOCAL_LLM_BACKEND || 'local',
            operation: 'embeddings-probe',
  ...
  async function createEmbedding(text = '', semanticStatus = null) {
    ...
    const response = await fetch(`${embeddingBase}/embeddings`, {
      ...
      body: JSON.stringify({
        ...
        input: formatEmbeddingRequestInput(String(text || ''), configuredEmbedModel),
      }),
    });
    const bodyText = await response.text();
    if (!response.ok) {
    ...
    if (existing?.vector?.length) {
      if (migrated && legacyHash && legacyHash !== hash) {
        ...
        store.items[hash] = {
          ...
          requestFormat: embeddingRequestFormatKeyForModel(model || configuredEmbedModel || ''),
        };
