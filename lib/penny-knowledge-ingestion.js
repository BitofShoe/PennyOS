...
function collectSourceArtifacts(options = {}) {
  const rawArtifacts = [];
  if (Array.isArray(options.sourceArtifacts)) rawArtifacts.push(...options.sourceArtifacts);
  if (options.sourceArtifact && typeof options.sourceArtifact === 'object') rawArtifacts.push(options.sourceArtifact);
  if (
    options.rawSourceText != null
    || options.rawSourceBytes != null
    || options.checksumSha256
  ) {
    rawArtifacts.push({
      ...(options.sourceArtifactMetadata && typeof options.sourceArtifactMetadata === 'object'
        ? options.sourceArtifactMetadata
        : {}),
      sourceType: options.sourceType || options.sourceArtifactType || 'conversation-export',
      rawSourceText: options.rawSourceText,
      rawSourceBytes: options.rawSourceBytes,
  ...
  let duplicateSourceArtifactCount = 0;
  const seen = new Set();
  for (const raw of rawArtifacts) {
    const artifact = normalizeSourceArtifact(raw);
    const key = `${artifact.sourceType}:${artifact.checksumSha256}`;
    if (seen.has(key)) {
      duplicateSourceArtifactCount += 1;
      continue;
    }
    seen.add(key);
    sourceArtifacts.push(artifact);
  }
  return { sourceArtifacts, duplicateSourceArtifactCount };
}
...
function chunkConversationThread(thread = {}, {
  ...
  const chunkSourceArtifactId = normalizeText(
    sourceArtifactId || normalizedThread.sourceArtifactId || '',
    180,
  );
  const messages = [];
  ...
  function flushChunk() {
    ...
    chunks.push(normalizeThreadChunk({
      ...
      sourceArtifactId: chunkSourceArtifactId,
      ordinal,
...
function ingestConversationThreads(rawThreads = [], options = {}) {
  ...
  const { sourceArtifacts, duplicateSourceArtifactCount } = collectSourceArtifacts(options);
  const primarySourceArtifactId = normalizeText(options.sourceArtifactId || sourceArtifacts[0]?.sourceId || '', 180);
  ...
  const threads = preparedThreads
    ...
    .map((item) => (primarySourceArtifactId && !item.sourceArtifactId
      ? { ...item, sourceArtifactId: primarySourceArtifactId }
      : item))
  ...
  const chunks = threads.flatMap((thread) => chunkConversationThread(thread, {
    ...options,
    sourceArtifactId: thread.sourceArtifactId || primarySourceArtifactId,
    validation,
  }));
