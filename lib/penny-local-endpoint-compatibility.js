...
async function postChat(fetchImpl, endpoint, payload, { apiKey = '' } = {}) {
...
async function probeLocalEndpointCompatibility({
  ...
  apiKey = process.env.PENNY_LMSTUDIO_API_KEY || '',
  ...
  if (probeModelCall) {
    ...
    try {
      const { response } = await postChat(fetch, base, tinyChatPayload({ model: probeModel }), { apiKey });
    ...
    try {
      const { response } = await postChat(fetch, base, tinyChatPayload({ role: 'developer', model: probeModel }), { apiKey });
      capabilities.developer_role = statusFromResponse(response);
    } catch (_err) {
      capabilities.developer_role = 'unsupported';
    }
    try {
      const { response } = await postChat(fetch, base, tinyChatPayload({ model: probeModel, extra: { reasoning_effort: 'low' } }), { apiKey });
      capabilities.reasoning_effort = statusFromResponse(response);
    } catch (_err) {
      capabilities.reasoning_effort = 'unsupported';
    }
    try {
      const { response } = await postChat(fetch, base, tinyChatPayload({ model: probeModel, extra: { stream: true } }), { apiKey });
