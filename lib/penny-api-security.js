...
function resolveBindHost({ lanShare = false, host = '' } = {}) {
  ...
  if (explicit) {
    if (lanShare === true || isLoopbackAddress(explicit)) return explicit;
    return '127.0.0.1';
  }
...
function createPennyApiSecurity({
  ...
  function isStrongTokenRoute(req, url) {
    ...
    if (method === 'POST' && (pathname === '/api/penny/chat' || pathname === '/api/companion/chat')) return true;
    if (method === 'POST' && pathname.startsWith('/api/penny/sidecars/')) return true;
    if (method === 'POST' && pathname === '/api/penny/lmstudio/model') return true;
    if (method === 'POST' && pathname === '/api/penny/provider/openai/connect') return true;
    if (method === 'POST' && pathname === '/api/penny/provider/local/reset') return true;
    if (method === 'POST' && pathname === '/api/penny/web-settings') return true;
    if (method === 'POST' && pathname === '/api/penny/voice/config') return true;
    if (method === 'POST' && pathname === '/api/penny/voice/speech') return true;
    if (method === 'GET' && pathname === '/api/penny/memory') return true;
    if (method === 'GET' && pathname === '/api/penny/memory/inspector') return true;
    if (method === 'GET' && pathname === '/api/penny/memory/export') return true;
    if (method !== 'GET' && pathname === '/api/penny/memory') return true;
  ...
  function startupSecurityLines({ port } = {}) {
    ...
    if (lanShare) {
      ...
      if (isStrongTokenRoute({ method: 'POST' }, { pathname: '/api/penny/memory/purge' })) {
        lines.push('Memory read, inspector, export, mutation, review, consolidation, workspace-write approval, and model-change routes require the local session token.');
      }
    }
