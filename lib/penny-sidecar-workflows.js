...
function downstreamUseGuardrails() {
  return {
    allowedUse: 'review-only',
    memoryWrite: false,
    promptTruthChanged: false,
    toolEvidenceReceiptChanged: false,
    defaultContextChanged: false,
...
function kindDefaults(kind = 'unknown') {
  if (kind === 'docs') {
    return {
      extractionMethod: 'document-rag-fixture',
      sourceFlag: 'fixture-docs-only',
      sensitivity: 'low',
      sourceAuthority: 'document-extraction',
    };
  }
  if (kind === 'audio') {
    return {
      extractionMethod: 'transcript-review',
      sourceFlag: 'fixture-transcript-only',
      sensitivity: 'private-audio-blocked',
      sourceAuthority: 'audio-transcript-review',
    };
  }
  return {
    extractionMethod: 'search-digest',
    sourceFlag: 'public-web-fixture',
    sensitivity: 'low',
    sourceAuthority: 'web-search-review',
  };
}

function buildPipelineProvenance({
  kind = 'unknown',
  route = '',
  mode = 'fixture',
  generatedAt = '',
  ...
  return {
    source: {
      kind: cleanText(kind, 'unknown'),
      route: cleanText(route),
      mode: normalizeMode(mode),
      generatedAt: cleanText(generatedAt),
      artifactSchema: cleanText(artifactSchema, 'unknown'),
      trialStatus: cleanText(trialStatus, 'unknown'),
      count: Math.max(0, Number(sourceCount) || 0),
      authorityDomain: defaults.sourceAuthority,
    },
    extraction: {
      method: cleanText(extractionMethod, defaults.extractionMethod),
      status: 'extracted',
    },
    cleaning: {
      status: 'normalized',
      transforms: ['trim-whitespace', 'bounded-review-fields'],
    },
    dedupe: {
      status: 'stable-order',
      strategy: 'source-key-stable-order',
      duplicateCount: 0,
    },
    quality: {
      supportScore: Math.max(0, Math.min(1, Number(supportScore) || 0)),
      supportState: 'review_required',
    },
    privacy: {
      flag: cleanText(privacyFlag, defaults.sourceFlag),
      sensitivity: cleanText(sensitivity, defaults.sensitivity),
    },
    review: {
      status: cleanText(reviewStatus, 'review_required'),
      requiresReview: true,
      autoIngested: false,
    },
    downstreamUse: downstreamUseGuardrails(),
  };
}

function buildSourcePipelineProvenance({
  kind = 'search',
  ...
  return {
    ...
    extraction: {
      method: cleanText(extractionMethod),
      status: 'extracted',
    },
    cleaning: {
      status: 'normalized',
  ...
  return (Array.isArray(digest.sources) ? digest.sources : [])
    .map((source, index) => {
      ...
      return {
        ...
        retrievedAt: cleanText(source?.retrieved_at || source?.retrievedAt),
        pipelineProvenance: buildSourcePipelineProvenance({
          kind: 'search',
          extractionMethod: 'search-result',
          privacyFlag: 'public-web-fixture',
          sensitivity: 'low',
          supportScore: 0.6,
        }),
      };
  ...
  return (Array.isArray(ragAnswer.document_citations) ? ragAnswer.document_citations : [])
    .map((citation, index) => {
      ...
      return {
        ...
        pipelineProvenance: buildSourcePipelineProvenance({
          kind: 'docs',
          extractionMethod: 'document-chunk',
          privacyFlag: 'fixture-docs-only',
          sensitivity: 'low',
          supportScore: 0.7,
        }),
      };
...
function blockedWorkflowReceipt({
  ...
  return {
    ...
    pipelineProvenance: buildPipelineProvenance({
      kind,
      route,
      mode: requestedMode,
      reviewStatus: 'permission_required',
      sourceCount: 0,
      supportScore: 0,
    }),
    sourceReceipts: [],
...
function runSearchSidecarWorkflow({
  ...
  return {
    ...
    pipelineProvenance: buildPipelineProvenance({
      ...
      privacyFlag: 'public-web-fixture',
      sensitivity: 'low',
      supportScore: sourceReceipts.length ? 0.6 : 0,
    }),
    review: {
      requiresReview: true,
      autoIngested: false,
...
function runDocsSidecarWorkflow({
  ...
  return {
    ...
    pipelineProvenance: buildPipelineProvenance({
      ...
      privacyFlag: 'fixture-docs-only',
      sensitivity: 'low',
      supportScore: sourceReceipts.length ? 0.7 : 0,
    }),
    review: {
      requiresReview: true,
      autoIngested: false,
...
function runAudioSidecarWorkflow({
  ...
  return {
    ...
    pipelineProvenance: buildPipelineProvenance({
      ...
      reviewStatus: 'review_required',
      extractionMethod: 'transcript-review',
      privacyFlag: 'fixture-transcript-only',
      sensitivity: 'private-audio-blocked',
      supportScore: 0.5,
    }),
    capture: {
