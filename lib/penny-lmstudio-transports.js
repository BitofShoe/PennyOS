...
} = require('./penny-lmstudio-response-limit');
const {
  createProviderError,
  isProviderError,
  toProviderError,
...
function isOpenAiCloudBase(value = '') {
  try {
...
function createLmStudioTransportApi({
  ...
  function providerError(code, operation, options = {}) {
  ...
  function normalizeProviderFailure(error, operation) {
  ...
  function classifyUpstreamReason(bodyText = '', { mayBeMissingThread = false } = {}) {
    const probe = { message: String(bodyText || '') };
    if (mayBeMissingThread && isMissingLmStudioThreadError(probe)) return 'missing_thread';
    if (/\b(model does not exist|model .*not found|unknown model|no such model)\b/i.test(probe.message)) {
      return 'missing_model';
    }
    return '';
  }

  function finalizeReasoningStream(accumulator, laneRuntime) {
    const snapshot = accumulator.snapshot();
    if (laneRuntime && typeof laneRuntime === 'object') {
      laneRuntime.reasoningStream = {
        capturedChars: snapshot.capturedChars,
  ...
  function appendFinalReasoningSnapshot(accumulator, value = '') {
    const text = String(value || '');
    if (!text) return;
    const snapshot = accumulator.snapshot();
    if (snapshot.totalChars === text.length && text.startsWith(snapshot.text)) return;
    if (!snapshot.truncated && text.startsWith(snapshot.text)) {
      accumulator.append(text.slice(snapshot.text.length));
      return;
    }
    if (!snapshot.truncated && snapshot.text.startsWith(text)) return;
    accumulator.append(text);
  }

  function resetStreamForRetry(onEvent, reason = 'transport-retry') {
    onEvent?.({ type: 'stream.reset', reason });
  }

  function beginReasoningReceipt(laneRuntime, transport) {
    recordReasoningRequest(laneRuntime, {
      state: 'not-requested',
      source: `${transport}-request-payload`,
      control: 'omitted',
    });
  }

  function finishReasoningReceipt(laneRuntime, transport, {
    reasoningChars = null,
    ...
    recordReasoningObservation(laneRuntime, {
      ...
      signal: Number(reasoningChars) > 0
        ? 'reasoning-content'
        : (Number(reasoningTokens) > 0 ? 'reasoning-tokens' : 'none'),
      reasoningChars,
      reasoningTokens,
      truncated,
    });
  ...
  function buildChatSamplingFields() {
    const backend = String(LOCAL_LLM_BACKEND || '').trim().toLowerCase();
    const omitLocalOnlySampling = isOpenAiCloudBase(LMSTUDIO_BASE) || backend === 'openai_cloud';
    ...
    return {
      ...
      ...(!omitLocalOnlySampling && sampling.topK !== null ? { top_k: sampling.topK } : {}),
    };
  }
  ...
  function salvageVisibleReplyDecision({ visibleText = '', reasoningText = '' } = {}) {
    ...
    if (ALLOW_RAW_REASONING_FALLBACK && reasoningText) {
  ...
  async function runLmStudioResponsesApi({ userText, messages, memories, file, abortSignal, lane = 'chat', laneRuntime, latencyBudget = null }) {
    return withLmStudioLaneModel(lane, async (model) => {
      beginReasoningReceipt(laneRuntime, 'responses');
      ...
      try {
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
          throw providerError('provider_upstream_error', 'responses', {
            upstreamStatus: response.statusCode,
            reasonCode: classifyUpstreamReason(bodyText),
          });
        }
        let parsed;
        try {
          ...
          throw providerError('provider_invalid_response', 'responses');
        }
        const { outputText, reasoningText } = collectLmStudioResponsesStrings(parsed);
        finishReasoningReceipt(laneRuntime, 'responses', {
        ...
        if (!primary.text) {
          throw providerError('provider_no_visible_text', 'responses');
        }
        recordLaneCleanup(laneRuntime, primary);
        return primary.text;
      } catch (error) {
        throw normalizeProviderFailure(error, 'responses');
      } finally {
        clearTimeout(timer);
      }
    }, laneRuntime);
  ...
  async function runLmStudioStatefulChatApi({ userText, messages, memories, image, file, abortSignal, lane = 'chat', laneRuntime, latencyBudget = null }) {
    return withLmStudioLaneModel(lane, async (model, status) => {
      beginReasoningReceipt(laneRuntime, 'native-stateful');
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), LMSTUDIO_TIMEOUT_MS);
      bindAbortSignal(controller, abortSignal);
      ...
      try {
        ...
        const bodyText = response.bodyText;
        if (response.statusCode < 200 || response.statusCode >= 300) {
          throw providerError('provider_upstream_error', 'stateful-chat', {
            upstreamStatus: response.statusCode,
            reasonCode: classifyUpstreamReason(bodyText, { mayBeMissingThread: canContinue }),
          });
        }
        let parsed;
        try {
          ...
          throw providerError('provider_invalid_response', 'stateful-chat');
        }

        const { responseId, outputText, reasoningText } = collectLmStudioStatefulChatStrings(parsed);
        finishReasoningReceipt(laneRuntime, 'native-stateful', {
          reasoningChars: reasoningText.length,
        });
        maybeReportReasoning({ transport: 'native-stateful', lane, model: nativeModel, reasoningText });
        const primary = salvageVisibleReplyDecision({ visibleText: outputText, reasoningText });
        if (!primary.text) {
          throw providerError('provider_no_visible_text', 'stateful-chat');
        }
        ...
        throw normalizeProviderFailure(error, 'stateful-chat');
      } finally {
        clearTimeout(timer);
      }
    }, laneRuntime);
  ...
  async function streamLmStudioStatefulChatApi({ userText, messages, memories, image, file, onEvent, abortSignal, lane = 'chat', laneRuntime, latencyBudget = null }) {
    return withLmStudioLaneModel(lane, async (model, status) => {
      beginReasoningReceipt(laneRuntime, 'native-stateful-stream');
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), LMSTUDIO_TIMEOUT_MS);
      const nativeModel = pickLmStudioNativeModelId(model, status);
      const systemPrompt = buildLmStudioLeanSystemPrompt({ memories, latencyBudget });
      ...
      try {
        ...
        const reasoningAccumulator = createBoundedTextAccumulator();
        let responseId = '';
        const emit = createStreamEventForwarder(onEvent);

        await postJsonSse(`${LMSTUDIO_NATIVE_BASE}/chat`, {
          provider: LOCAL_LLM_BACKEND || 'lm_studio',
          operation: 'stateful-chat-stream',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${LMSTUDIO_API_KEY}`,
          },
          ...
          onEvent: ({ event, data }) => {
            if (event === 'message.delta') {
              ...
              if (chunk) {
                ...
                emit({ type: 'message.delta', content: chunk, text: visibleText });
              }
              return;
            }
            if (event === 'reasoning.delta') {
              const chunk = typeof data?.content === 'string' ? data.content : '';
              if (chunk) reasoningAccumulator.append(chunk);
            ...
            if (event === 'chat.end') {
              ...
              appendFinalReasoningSnapshot(reasoningAccumulator, collected.reasoningText);
              return;
            }
            if (event === 'error') {
              const detail = typeof data?.error === 'string'
                ...
                : typeof data?.message === 'string'
                  ? data.message
                  : JSON.stringify(data);
              throw providerError('provider_stream_error', 'stateful-chat-stream');
            }
            const label = lmStudioStageLabel(event);
            if (label) emit({ type: 'status', stage: event, label });
          },
        });

        const reasoningText = finalizeReasoningStream(reasoningAccumulator, laneRuntime);
        finishReasoningReceipt(laneRuntime, 'native-stateful-stream', {
          reasoningChars: laneRuntime?.reasoningStream?.totalChars ?? reasoningText.length,
          truncated: laneRuntime?.reasoningStream?.truncated === true,
        });
        ...
        if (!primary.text) throw providerError('provider_no_visible_text', 'stateful-chat-stream');
        ...
        if (error?.name === 'AbortError' && abortSignal?.aborted) throw error;
        if (canContinue && isMissingLmStudioThreadError(error)) {
          if (memories && typeof memories === 'object') memories.lmStudioThread = null;
          resetStreamForRetry(onEvent, 'stale-thread-retry');
          return streamLmStudioStatefulChatApi({ userText, messages, memories, image, file, onEvent, abortSignal, lane, laneRuntime });
        }
        throw normalizeProviderFailure(error, 'stateful-chat-stream');
      } finally {
        clearTimeout(timer);
      }
    }, laneRuntime);
  ...
  async function streamLmStudioResponsesApi({ userText, messages, memories, file, onEvent, abortSignal, lane = 'chat', laneRuntime, latencyBudget = null }) {
    return withLmStudioLaneModel(lane, async (model) => {
      beginReasoningReceipt(laneRuntime, 'responses-stream');
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), LMSTUDIO_TIMEOUT_MS);
      bindAbortSignal(controller, abortSignal);
      ...
      try {
          ...
          onEvent: ({ event, data }) => {
            ...
            if (type === 'response.output_text.delta') {
              ...
              if (chunk) {
                ...
                if (!replyStarted) {
                  replyStarted = true;
                  emit({ type: 'status', stage: 'message.start', label: 'replying' });
                }
                emit({ type: 'message.delta', content: chunk, text: visibleText });
              }
              return;
            ...
            if (type === 'response.in_progress' || type === 'response.created') {
              emit({ type: 'status', stage: type, label: 'thinking' });
              return;
            }

            if (type === 'response.output_item.added' || type === 'response.content_part.added') {
              emit({ type: 'status', stage: type, label: 'replying' });
              return;
            }
            ...
            if (type === 'response.failed' || type === 'error') {
              const detail = typeof data?.error === 'string'
                ...
                : typeof data?.message === 'string'
                  ? data.message
                  : JSON.stringify(data);
              throw providerError('provider_stream_error', 'responses-stream');
            }
          },
        });

        const reasoningText = finalizeReasoningStream(reasoningAccumulator, laneRuntime);
        finishReasoningReceipt(laneRuntime, 'responses-stream', {
          reasoningChars: laneRuntime?.reasoningStream?.totalChars ?? reasoningText.length,
          truncated: laneRuntime?.reasoningStream?.truncated === true,
        });
        ...
        if (!primary.text && RESPONSES_THEN_CHAT_FALLBACK) {
          resetStreamForRetry(onEvent);
          return streamLmStudioChatCompletionsApi({ userText, messages, memories, file, onEvent, abortSignal, lane, laneRuntime });
        }
        if (!primary.text) {
          throw providerError('provider_no_visible_text', 'responses-stream');
        }
        recordLaneCleanup(laneRuntime, primary);
        return primary.text;
      } catch (error) {
        if (error?.name === 'AbortError' && abortSignal?.aborted) throw error;
        throw normalizeProviderFailure(error, 'responses-stream');
      } finally {
        clearTimeout(timer);
      }
    }, laneRuntime);
  ...
  async function streamLmStudioChatCompletionsApi({ userText, messages, memories, image, file, onEvent, abortSignal, lane = 'chat', laneRuntime, latencyBudget = null }) {
    return withLmStudioLaneModel(lane, async (model) => {
      beginReasoningReceipt(laneRuntime, 'chat-completions-stream');
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), LMSTUDIO_TIMEOUT_MS);
      bindAbortSignal(controller, abortSignal);
      ...
      try {
        ...
        await postJsonSse(`${LMSTUDIO_BASE}/chat/completions`, {
          ...
          onEvent: ({ event, data }) => {
              ...
              const detail = typeof data?.error === 'string'
                ...
                : typeof data?.error?.message === 'string'
                  ...
                  : typeof data?.message === 'string'
                    ? data.message
                    : JSON.stringify(data);
              throw providerError('provider_stream_error', 'chat-completions-stream');
            }
            if (typeof data === 'string') {
              if (data === '[DONE]') return;
              if (data) throw providerError('provider_stream_error', 'chat-completions-stream');
              return;
            }
            ...
            if (contentChunk) {
              ...
              emit({ type: 'message.delta', content: contentChunk, text: visibleText });
            }
          },
        });
        ...
        if (!primary.text) throw providerError('provider_no_visible_text', 'chat-completions-stream');
        clearLmStudioThread(memories);
        recordLaneCleanup(laneRuntime, primary);
        recordLmStudioResponseLimitTelemetry(laneRuntime, normalizeLmStudioResponseLimitTelemetry({
          finishReason,
          usage,
          payload,
        ...
        finishReasoningReceipt(laneRuntime, 'chat-completions-stream', {
          reasoningChars: laneRuntime?.reasoningStream?.totalChars ?? reasoningText.length,
          reasoningTokens: laneRuntime?.responseLimit?.reasoningTokens ?? null,
          truncated: laneRuntime?.reasoningStream?.truncated === true,
        });
        return primary.text;
      } catch (error) {
        if (error?.name === 'AbortError' && abortSignal?.aborted) throw error;
        throw normalizeProviderFailure(error, 'chat-completions-stream');
      } finally {
        clearTimeout(timer);
      }
    }, laneRuntime);
  ...
  async function runLmStudioChatCompletionsApi({
    userText,
    messages,
    memories,
    image,
    ...
    return withLmStudioLaneModel(lane, async (model) => {
      beginReasoningReceipt(laneRuntime, 'chat-completions');
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), LMSTUDIO_TIMEOUT_MS);
      bindAbortSignal(controller, abortSignal);
      try {
        ...
        const bodyText = response.bodyText;
        if (response.statusCode < 200 || response.statusCode >= 300) {
          throw providerError('provider_upstream_error', 'chat-completions', {
            upstreamStatus: response.statusCode,
          });
        }
        let parsed;
        try {
          ...
          throw providerError('provider_invalid_response', 'chat-completions');
        }
        ...
        if (!decision.text) throw providerError('provider_no_visible_text', 'chat-completions');
        if (!preserveThread) clearLmStudioThread(memories);
        recordLaneCleanup(laneRuntime, decision);
        recordLmStudioResponseLimitTelemetry(laneRuntime, extractLmStudioResponseLimitTelemetry({
          parsed,
          payload,
          source: 'chat-completions',
          lane,
        }));
        finishReasoningReceipt(laneRuntime, 'chat-completions', {
          reasoningChars: reasoningText.length,
          reasoningTokens: laneRuntime?.responseLimit?.reasoningTokens ?? null,
        });
        return decision.text.trim();
      } catch (error) {
        throw normalizeProviderFailure(error, 'chat-completions');
      } finally {
        clearTimeout(timer);
      }
    }, laneRuntime);
  ...
  async function runLmStudioLocal({ userText, messages, memories, image, file, abortSignal, lane = 'chat', laneRuntime, latencyBudget = null }) {
    ...
    try {
      ...
      if (error?.name === 'AbortError' || error?.code === 'provider_timeout' || /timed out/i.test(msg)) throw error;
      if (
        code === 404
        || error?.code === 'provider_upstream_error'
        || error?.code === 'provider_no_visible_text'
        || /404/.test(msg)
        || /not found/i.test(msg)
        || /No assistant text from LM Studio stateful chat/i.test(msg)
        || /LM Studio stateful chat error/i.test(msg)
      ) {
        clearLmStudioThread(memories);
        try {
          ...
          const chatCode = chatError?.upstreamStatus || chatError?.statusCode;
  ...
  async function streamLmStudioLocal({ userText, messages, memories, image, file, onEvent, abortSignal, lane = 'chat', laneRuntime, latencyBudget = null }) {
    ...
    try {
      ...
      if (
        ...
        || /404/.test(msg)
        || /not found/i.test(msg)
        || /No assistant text from LM Studio stateful chat stream/i.test(msg)
        || /LM Studio stateful chat stream error/i.test(msg)
      ) {
        clearLmStudioThread(memories);
        resetStreamForRetry(onEvent);
        try {
