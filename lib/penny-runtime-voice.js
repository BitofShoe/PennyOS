...
function isLoopbackSpeachesHost(hostname = '') {
...
function normalizeBaseUrl(value = '') {
  ...
  try {
    ...
    throw new RuntimeVoiceError(400, 'invalid_base_url', 'Speaches base URL must be a valid local HTTP URL.');
  }
  if (!['http:', 'https:'].includes(parsed.protocol)) {
    throw new RuntimeVoiceError(400, 'invalid_base_url', 'Speaches base URL must use http or https.');
  }
  if (parsed.username || parsed.password) {
    throw new RuntimeVoiceError(400, 'invalid_base_url', 'Speaches base URL must not include credentials.');
  }
  if (!isLoopbackSpeachesHost(parsed.hostname)) {
    throw new RuntimeVoiceError(400, 'invalid_base_url', 'Speaches base URL must point at localhost or 127.0.0.1.');
  }
  parsed.hash = '';
  parsed.search = '';
  parsed.pathname = parsed.pathname.replace(/\/+$/, '').replace(/\/v1$/i, '') || '/';
  return parsed.toString().replace(/\/+$/, '') || DEFAULT_SPEACHES_BASE_URL;
}
...
function normalizeSpeechSpeed(value = DEFAULT_SPEECH_SPEED) {
  const number = Number(value);
  if (!Number.isFinite(number)) return DEFAULT_SPEECH_SPEED;
  return Math.max(0.25, Math.min(4, number));
}

function applyPcm16WavGain(audioBuffer, gain = 1) {
  const safeGain = normalizeAudioGain(gain);
  if (safeGain <= 1.001 || !Buffer.isBuffer(audioBuffer) || audioBuffer.length < 44) return audioBuffer;
  ...
  while (offset + 8 <= audioBuffer.length) {
    ...
    if (chunkStart + chunkSize > audioBuffer.length) break;
  ...
  const boosted = Buffer.from(audioBuffer);
  const dataEnd = Math.min(boosted.length, dataOffset + dataSize);
  for (let sampleOffset = dataOffset; sampleOffset + 1 < dataEnd; sampleOffset += 2) {
    const sample = boosted.readInt16LE(sampleOffset);
    const scaled = Math.max(-32768, Math.min(32767, Math.round(sample * safeGain)));
    boosted.writeInt16LE(scaled, sampleOffset);
  }
  return boosted;
}
...
function normalizeRuntimeVoiceConfig(input = {}, fallback = {}) {
  ...
  return {
    ...
    speed: normalizeSpeechSpeed(
      source.speed
      || base.speed
      || process.env.PENNY_SPEACHES_SPEED
      || process.env.SPEACHES_SPEED
...
function createRuntimeVoiceApi({
  ...
  async function synthesizeSpeech({ text = '', voice = '', gain = 1, speed = null } = {}) {
    const cfg = getConfig();
    ...
    const payload = {
      ...
      speed: normalizeSpeechSpeed(speed ?? cfg.speed),
    };
    ...
    const audioBuffer = applyPcm16WavGain(Buffer.from(await response.arrayBuffer()), gain);
