...
function createProjectToolsApi({
  ...
  pendingWorkspaceWritesFile = null,
  now = () => Date.now(),
} = {}) {
  ...
  if (!fs || typeof fs.readFileSync !== 'function' || typeof fs.lstatSync !== 'function') throw new TypeError('createProjectToolsApi requires fs');
  ...
  const pendingWorkspaceWritesPath = pendingWorkspaceWritesFile
    ? path.resolve(pendingWorkspaceWritesFile)
    : path.join(normalizedProjectRoot, 'data', 'penny-pending-workspace-writes.json');
  ...
  function containmentRootForPath(candidatePath) {
    const resolved = path.resolve(candidatePath);
    for (const { alias, root } of normalizedPathAliases) {
      if (isPathInsideRoot(resolved, root)) {
        return {
          root,
          error: `Path must stay inside the ${alias} alias.`,
        };
      }
    }
    return {
      root: normalizedProjectRoot,
      error: 'Path must stay inside the Penny project.',
    };
  }

  function assertRealPathInsideConfiguredRoot(candidatePath) {
    const { root, error } = containmentRootForPath(candidatePath);
    if (!isRealPathInsideRoot(root, candidatePath, { fsModule: fs, pathModule: path })) {
  ...
  function resolveProjectPath(inputPath = '.') {
    ...
    const aliasPath = normalizedRaw.replace(/^(\.\/)+/, '');
    ...
    if (alias) {
      ...
      assertRealPathInsideConfiguredRoot(resolvedAliasPath);
    ...
    assertRealPathInsideConfiguredRoot(resolved);
    return resolved;
  }
  ...
  function policyRelativePaths(filePath) {
    const candidates = [path.resolve(filePath)];
    if (fs.existsSync(filePath)) {
      try {
        candidates.push(resolveRealPathForContainment(filePath, { fsModule: fs, pathModule: path }));
      } catch {
        // The configured-root assertion reports an escaping or unreadable target separately.
      }
    }
    return [...new Set(candidates.map((candidate) => toProjectRelative(candidate).replace(/\\/g, '/')))];
  }

  function isSecretBearingProjectFile(filePath) {
    return policyRelativePaths(filePath).some((rel) => {
  ...
  function assertReadableProjectFile(filePath) {
    assertRealPathInsideConfiguredRoot(filePath);
    if (isSecretBearingProjectFile(filePath)) {
      throw new Error(`${toProjectRelative(filePath)} is a secret-bearing file and is not readable through Penny project tools.`);
    }
  }
  ...
  function serializePendingWorkspaceWrite(pending) {
    return {
      id: pending.id,
      path: pending.path,
  ...
  function persistPendingWorkspaceWrites() {
    const payload = {
      schema: 'penny-pending-workspace-writes.v1',
      updatedAt: new Date(now()).toISOString(),
      pending: [...pendingWorkspaceWrites.values()].map(serializePendingWorkspaceWrite),
    };
    fs.mkdirSync(path.dirname(pendingWorkspaceWritesPath), { recursive: true });
    const tmpPath = `${pendingWorkspaceWritesPath}.${process.pid}.${crypto.randomBytes(4).toString('hex')}.tmp`;
    fs.writeFileSync(tmpPath, `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
    fs.renameSync(tmpPath, pendingWorkspaceWritesPath);
  }

  function normalizeStoredPendingWorkspaceWrite(rawPending = {}) {
    const id = String(rawPending.id || '').trim();
    const storedPath = String(rawPending.path || '').trim();
    if (!id || !storedPath) return null;
    const expiresAt = Number(rawPending.expiresAt || 0);
    if (!Number.isFinite(expiresAt) || expiresAt <= now()) return null;
    const filePath = resolveProjectPath(storedPath);
    assertWritableProjectFile(filePath);
    return {
      id,
      filePath,
      path: toProjectRelative(filePath),
      action: String(rawPending.action || 'updated'),
      operation: String(rawPending.operation || 'write_project_file'),
      before: String(rawPending.before || ''),
      after: String(rawPending.after || ''),
      baseHash: String(rawPending.baseHash || sha256Text(String(rawPending.before || ''))),
      nextHash: String(rawPending.nextHash || sha256Text(String(rawPending.after || ''))),
      bytes: Number(rawPending.bytes || Buffer.byteLength(String(rawPending.after || ''), 'utf8')),
      lines: Number(rawPending.lines || countLines(String(rawPending.after || ''))),
      createdAt: Number(rawPending.createdAt || now()),
      expiresAt,
      patch: String(rawPending.patch || ''),
      summary: String(rawPending.summary || `pending approval for ${storedPath}`),
      metadata: rawPending.metadata && typeof rawPending.metadata === 'object' && !Array.isArray(rawPending.metadata)
        ? { ...rawPending.metadata }
        : {},
    };
  }

  function loadPendingWorkspaceWrites() {
    if (!fs.existsSync(pendingWorkspaceWritesPath)) return;
    let shouldPersist = false;
    try {
      const parsed = JSON.parse(fs.readFileSync(pendingWorkspaceWritesPath, 'utf8'));
      const entries = Array.isArray(parsed?.pending) ? parsed.pending : [];
      for (const entry of entries) {
        const normalized = normalizeStoredPendingWorkspaceWrite(entry);
        if (normalized) {
          pendingWorkspaceWrites.set(normalized.id, normalized);
        } else {
          shouldPersist = true;
        }
      }
      if (pendingWorkspaceWrites.size !== entries.length) shouldPersist = true;
    } catch {
      shouldPersist = true;
    }
    if (shouldPersist) persistPendingWorkspaceWrites();
  }
  ...
  function readProjectFileTool(args = {}) {
    ...
    assertReadableProjectFile(filePath);
  ...
  function searchProjectTextTool(args = {}) {
    ...
    while (queue.length && hits.length < limit) {
      ...
      if (isSecretBearingProjectFile(current)) continue;
  ...
  function assertNoSymlinkSegmentsForWrite(filePath) {
    const { root } = containmentRootForPath(filePath);
    ...
    for (const segment of segments) {
      ...
      if (fs.lstatSync(probe).isSymbolicLink()) {
        throw new Error(`${toProjectRelative(filePath)} is a symlink path and cannot be edited through Penny project tools.`);
      }
    }
  }

  function assertWritableProjectFile(filePath) {
    assertRealPathInsideConfiguredRoot(filePath);
    assertNoSymlinkSegmentsForWrite(filePath);
    ensureWritableTextPath(filePath);
    if (isSecretBearingProjectFile(filePath)) {
      throw new Error(`${toProjectRelative(filePath)} is a secret-bearing file and cannot be edited through Penny project tools.`);
    }
  }
  ...
  function pruneExpiredPendingWorkspaceWrites() {
    ...
    for (const [id, pending] of pendingWorkspaceWrites.entries()) {
      if (Number(pending.expiresAt || 0) <= timestamp) {
        pendingWorkspaceWrites.delete(id);
        changed = true;
      }
    }
    if (changed) persistPendingWorkspaceWrites();
  }
  ...
  function replaceInProjectFileTool(args = {}) {
    ...
    const next = replaceAll
      ...
      : `${content.slice(0, firstIndex)}${replace}${content.slice(firstIndex + find.length)}`;
  ...
  function approvePendingWorkspaceWriteTool(args = {}) {
    ...
    assertWritableProjectFile(pending.filePath);
  ...
  loadPendingWorkspaceWrites();

  return {
    toProjectRelative,
    resolveProjectPath,
    isProbablyTextFile,
    isSecretBearingProjectFile,
