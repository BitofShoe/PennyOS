...
function normalizeReadiness(value = {}, defaults = {}) {
  ...
  const availabilityRaw = {
    ...(fallback.availability && typeof fallback.availability === 'object' ? fallback.availability : {}),
    ...(raw.availability && typeof raw.availability === 'object' ? raw.availability : {}),
  };
  const compatibilityRaw = {
    ...(fallback.compatibilityFallback && typeof fallback.compatibilityFallback === 'object'
      ? fallback.compatibilityFallback
      : {}),
    ...(raw.compatibilityFallback && typeof raw.compatibilityFallback === 'object'
      ? raw.compatibilityFallback
      : {}),
  };
  const semanticRaw = {
    ...(fallback.semanticDegradation && typeof fallback.semanticDegradation === 'object'
      ? fallback.semanticDegradation
      : {}),
    ...(raw.semanticDegradation && typeof raw.semanticDegradation === 'object'
      ? raw.semanticDegradation
      : {}),
  };
  const degradationRaw = {
    ...(fallback.degradation && typeof fallback.degradation === 'object' ? fallback.degradation : {}),
    ...(raw.degradation && typeof raw.degradation === 'object' ? raw.degradation : {}),
  };
  ...
  const chatModelReady = availabilityRaw.chatReady === true
    || raw.chatModelReady === true
    || fallback.chatModelReady === true;
  const toolModelReady = availabilityRaw.toolReady === true
    || raw.toolModelReady === true
    || fallback.toolModelReady === true;
  const embeddingReady = availabilityRaw.embeddingReady === true
    || raw.embeddingReady === true
    || fallback.embeddingReady === true;
  const compatibilityFallbackActive = compatibilityRaw.active === true;
  const semanticDegradationActive = semanticRaw.active === true;
  const explicitDegradation = degradationRaw.active === true;
  const legacyFallbackInput = raw.fallbackActive === true || fallback.fallbackActive === true;
  const hasExplicitReadinessCategories = !!(
    raw.availability
    || raw.compatibilityFallback
    || raw.semanticDegradation
    || raw.degradation
  );
  const legacyFallbackUnclassified = legacyFallbackInput && !hasExplicitReadinessCategories;
  const fallbackActive = legacyFallbackInput || compatibilityFallbackActive || semanticDegradationActive;
  const degradationReasons = uniqueStrings([
    ...(Array.isArray(degradationRaw.reasons) ? degradationRaw.reasons : []),
    ...(!chatModelReady ? ['chat-unavailable'] : []),
    ...(!toolModelReady ? ['tool-unavailable'] : []),
    ...(compatibilityFallbackActive ? ['compatibility-fallback'] : []),
    ...(semanticDegradationActive ? ['semantic-degradation'] : []),
    ...(legacyFallbackUnclassified
      ? ['legacy-fallback-active']
      : []),
  ], 12);
  const degradationActive = explicitDegradation || degradationReasons.length > 0;
  const readinessState = String(raw.readinessState || fallback.readinessState || '').trim()
    || (!chatModelReady || !toolModelReady
      ? 'unavailable'
      : (compatibilityFallbackActive
        ? 'compatibility-fallback'
        : (semanticDegradationActive ? 'ready-with-semantic-degradation' : 'ready')));
  ...
  return {
    schema: 'penny-runtime-readiness.v2',
    readinessState,
    availability: {
      providerReachable: availabilityRaw.providerReachable === true,
      chatReady: chatModelReady,
      toolReady: toolModelReady,
      embeddingReady,
      requiredReady: availabilityRaw.requiredReady === true
        || (chatModelReady && toolModelReady),
    },
    compatibilityFallback: {
      active: compatibilityFallbackActive,
      chat: compatibilityRaw.chat === true,
      tool: compatibilityRaw.tool === true,
      reasons: uniqueStrings(compatibilityRaw.reasons, 8),
    },
    semanticDegradation: {
      active: semanticDegradationActive,
      mode: String(semanticRaw.mode || (embeddingReady ? 'semantic' : 'unknown')).trim() || 'unknown',
      required: semanticRaw.required === true,
      reason: trimText(semanticRaw.reason || '', 180),
    },
    degradation: {
      active: degradationActive,
      reasons: degradationReasons,
    },
    legacyFallbackProjection: {
      ...
      deprecatedField: 'fallbackActive',
    },
    chatModelReady,
    toolModelReady,
    embeddingReady,
...
function buildStatusRuntimeContractReceipt({
  lmStudio = {},
  semanticMemory = {},
  generatedAt = new Date().toISOString(),
} = {}) {
  const status = lmStudio && typeof lmStudio === 'object' ? lmStudio : {};
  const semantic = semanticMemory && typeof semanticMemory === 'object' ? semanticMemory : {};
  const endpoint = trimText(
    status.localEndpointBase
      || status.endpoint
      || status.base
      || status.lmStudioBase
      || status.url
  ...
  const backend = trimText(
    status.localLlmBackend
      || status.backendFamily
      || status.backend_family
      || status.backend
      || '',
    120,
  );
  const loadedModels = uniqueStrings([
    ...
    ...(Array.isArray(status.loaded_models) ? status.loaded_models : []),
    ...(Array.isArray(status.availableModels) ? status.availableModels : []),
    ...(Array.isArray(status.installedModels) ? status.installedModels : []),
  ], 32);
  const resolvedModel = trimText(
    status.resolvedChatModel
      || status.resolvedModel
      || status.resolved_model_id
      || status.chatPreferredModel
      || status.configuredChatModel
  ...
  const capabilities = status.capabilities && typeof status.capabilities === 'object'
    ? status.capabilities
    : {};
  const contextLength = status.contextLength
    ?? status.context_length
    ?? status.maxContext
    ?? status.max_context_advertised
    ?? null;
  const cloudEndpoint = endpoint
    && !/(^https?:\/\/(?:127\.0\.0\.1|localhost|\[::1\])|^https?:\/\/10\.|^https?:\/\/192\.168\.|^https?:\/\/172\.(?:1[6-9]|2\d|3[0-1])\.)/i.test(endpoint);
  return buildRuntimeContractReceipt({
    generatedAt,
    endpointCompatibility: {
      endpoint,
      read_only: true,
      model_calls: false,
      backend_family: backend || 'unknown',
      loaded_models: loadedModels,
      resolved_model_id: resolvedModel,
      capabilities,
      max_context_advertised: contextLength,
    },
    endpoint,
    backend: backend || 'unknown',
    requestedModelId: resolvedModel,
    modelId: resolvedModel,
    fileFormat: status.fileFormat || status.file_format || 'unknown',
    quant: status.quant || status.quantization || 'unknown',
    contextLength,
    chatTemplate: status.chatTemplate || status.chat_template || 'unknown',
    promptTemplate: status.promptTemplate || status.prompt_template || status.chatTemplate || status.chat_template || 'unknown',
    samplingDefaults: status.samplingDefaults || status.sampling_defaults || {},
    capabilities,
    memoryLaneStatus: {
      status: semantic.mode || (semantic.ready === true ? 'semantic' : 'keyword'),
      semanticReady: semantic.ready === true,
      semanticKnown: typeof semantic.ready === 'boolean',
      requestedEmbeddingModel: semantic.configuredModel || semantic.configured_model || status.embedPreferredModel || status.configuredEmbedModel || '',
      privacyFlag: 'local-only',
      reviewStatus: 'runtime-status-only',
      downstreamUse: 'memory-inspector-readiness-only',
    },
    privacyWarningState: cloudEndpoint ? 'cloud-warning-required' : 'local-first',
    smokePrompt: {
      status: 'not_run',
      modelCall: false,
      reason: 'Memory inspector runtime contract receipt is status-only; no smoke model call was run.',
    },
    statePreservation: {
      model_state_preserved: true,
      started_models: false,
      stopped_models: false,
      loaded_models: false,
      unloaded_models: false,
      swapped_models: false,
...
function buildRuntimeReadiness({ lmStudio = {}, semanticMemory = {} } = {}) {
  ...
  const resolvedToolModel = String(lmStudio?.resolvedToolModel || '').trim();
  const requestedChatModel = String(lmStudio?.chatPreferredModel || lmStudio?.configuredChatModel || '').trim();
  const requestedToolModel = String(lmStudio?.toolPreferredModel || lmStudio?.configuredToolModel || '').trim();
  const chatCompatibilityFallback = !!(
    chatModelReady
    && requestedChatModel
    && resolvedChatModel
    && !modelsLookCompatible(requestedChatModel, resolvedChatModel)
  );
  const toolCompatibilityFallback = !!(
    toolModelReady
    && requestedToolModel
    && resolvedToolModel
    && !modelsLookCompatible(requestedToolModel, resolvedToolModel)
  );
  const compatibilityFallbackActive = chatCompatibilityFallback || toolCompatibilityFallback;
  const semanticDegradationActive = semanticMemory?.fallback === true
    || semanticMemory?.ready === false;
  const fallbackActive = !chatModelReady
    || !toolModelReady
    || compatibilityFallbackActive
    || semanticDegradationActive;
  ...
  return normalizeReadiness({
    ...
    fallbackActive,
    availability: {
      providerReachable: lmStudio?.reachable === true,
      chatReady: chatModelReady,
      toolReady: toolModelReady,
      embeddingReady,
      requiredReady: chatModelReady && toolModelReady,
    },
    compatibilityFallback: {
      active: compatibilityFallbackActive,
      chat: chatCompatibilityFallback,
      tool: toolCompatibilityFallback,
      reasons: [
        ...(chatCompatibilityFallback ? ['chat-model-mismatch'] : []),
        ...(toolCompatibilityFallback ? ['tool-model-mismatch'] : []),
      ],
    },
    semanticDegradation: {
      active: semanticDegradationActive,
      mode: String(semanticMemory?.mode || (embeddingReady ? 'semantic' : 'keyword-fallback')).trim(),
      required: semanticMemory?.required === true,
      reason: String(semanticMemory?.reason || '').trim(),
    },
    degradation: {
      active: fallbackActive,
      reasons: [
...
function buildRuntimeArtifact({
  ...
  const normalizedReasoningContract = normalizeReasoningContract(reasoningContract);
  ...
  return normalizeRuntimeArtifact({
    ...
    performance,
    readiness,
    reasoningContract: normalizedReasoningContract,
    modelAdvisory: {
      ...
      shadowError: shadowError ? 'Shadow provider request failed.' : '',
...
function normalizeRuntimeArtifact(value = {}, defaults = {}) {
  ...
  const reasoningContract = normalizeReasoningContract(raw.reasoningContract, fallback.reasoningContract);
  ...
  return {
    ...
    modelAdvisory: {
      ...
      shadowError: advisoryRaw.shadowError ? 'Shadow provider request failed.' : '',
...
function normalizeLastRouteInfo(value) {
  ...
  const reasoningContract = normalizeReasoningContract(value.reasoningContract);
...
function buildCombinedMemoryInspector({
  ...
  const runtime = {
    ...
    runtimeContract: buildStatusRuntimeContractReceipt({
      lmStudio,
      semanticMemory: inspector?.embeddings?.semanticMemory || {},
    }),
  };
...
module.exports = {
  ...
  normalizeReadiness,
  buildStatusRuntimeContractReceipt,
