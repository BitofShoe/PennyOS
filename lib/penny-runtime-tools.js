function createRuntimeToolsApi({
  ...
  if (typeof assertReadableProjectFile !== 'function') throw new TypeError('createRuntimeToolsApi requires assertReadableProjectFile');
  ...
  function resolveLogTarget(target = 'latest') {
    ...
    const known = {
      latest: null,
      stdout: path.join(projectRoot, 'penny-server.out.log'),
      stderr: path.join(projectRoot, 'penny-server.err.log'),
      server: path.join(projectRoot, 'penny-server.out.log'),
      'legacy-stdout': path.join(projectRoot, 'lyra-server.out.log'),
      'legacy-stderr': path.join(projectRoot, 'lyra-server.err.log'),
    };
    if (known[raw]) return known[raw];
    if (raw !== 'latest' && raw !== 'server') {
      const direct = resolveProjectPath(target);
      if (fs.existsSync(direct)) {
        assertReadableProjectFile(direct);
        const rel = toProjectRelative(direct).replace(/\\/g, '/');
        if (!rel.startsWith('logs/') || !/\.log$/i.test(rel)) {
          throw new Error('read_recent_logs only accepts Penny log files in logs/ or the named Penny server logs.');
        }
        return direct;
      }
    }
    const candidates = [
      path.join(projectRoot, 'penny-server.out.log'),
      path.join(projectRoot, 'penny-server.err.log'),
      ...
      ...(() => {
        ...
        return fs.readdirSync(logsDir)
          ...
          .filter((filePath) => fs.existsSync(filePath) && fs.statSync(filePath).isFile() && /\.log$/i.test(filePath));
      })(),
  ...
  function readRecentLogsTool(args = {}) {
    ...
    assertReadableProjectFile(logPath);
