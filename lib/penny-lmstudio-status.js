  ...
  async function getLmStudioConnectionStatus({ force = false } = {}) {
    ...
    try {
      ...
      let detail = error?.name === 'AbortError'
        ? `${localRuntimeLabel} models request timed out after ${timeoutMs}ms`
        : rawMsg;
      if (localLlmBackend === 'lm_studio' && settings?.enableLocalService === false) {
        ...
        if (code === 'ECONNREFUSED' || /\bfetch failed\b/i.test(rawMsg)) {
          detail = `Cannot reach ${LMSTUDIO_BASE} (${rawMsg}). Start ${localRuntimeLabel}'s OpenAI-compatible server and load a chat model, or set PENNY_LMSTUDIO_BASE if the port changed.`;
        }
      }
      value = {
        ok: false,
