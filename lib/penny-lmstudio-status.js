...
function createLmStudioStatusApi({
  ...
  LOCAL_RUNTIME_LABEL = 'LM Studio',
  LOCAL_ENDPOINT_BASE = LMSTUDIO_BASE,
  PENNY_LMSTUDIO_CHAT_MODEL,
  PENNY_LMSTUDIO_TOOL_MODEL,
  PENNY_LMSTUDIO_RUNTIME_PREFERRED_MODEL = '',
  PENNY_LMSTUDIO_RUNTIME_PREFERRED_TOOL_MODEL = '',
  PENNY_LMSTUDIO_DISABLE_MODEL_FALLBACK = false,
  PENNY_LMSTUDIO_DISABLE_CLI_DISCOVERY = false,
} = {}) {
  ...
  let modelFallbackDisabled = PENNY_LMSTUDIO_DISABLE_MODEL_FALLBACK === true
    || ['1', 'true', 'yes', 'on'].includes(String(PENNY_LMSTUDIO_DISABLE_MODEL_FALLBACK || '').trim().toLowerCase());
  const lmStudioCliDiscoveryDisabled = PENNY_LMSTUDIO_DISABLE_CLI_DISCOVERY === true
    || ['1', 'true', 'yes', 'on'].includes(String(PENNY_LMSTUDIO_DISABLE_CLI_DISCOVERY || '').trim().toLowerCase());
  ...
  let runtimePreferredToolModel = String(PENNY_LMSTUDIO_RUNTIME_PREFERRED_TOOL_MODEL || '').trim();
  ...
  function getPreferredModelForLane(lane = 'chat') {
    if (lane === 'tool') {
      return runtimePreferredToolModel
        || (modelFallbackDisabled && runtimePreferredChatModel
  ...
  async function getInstalledLmStudioModels() {
    if (lmStudioCliDiscoveryDisabled) return [];
    try {
  ...
  function buildLmStudioLaunchHint() {
    ...
    const parts = [];
    if (localLlmBackend === 'lm_studio' && settings?.enableLocalService === false) {
    ...
    if (localLlmBackend === 'lm_studio') {
      parts.push('In LM Studio, start the local server and keep at least one chat model loaded.');
    } else {
      parts.push(`Start ${localRuntimeLabel} and keep at least one chat model available.`);
    }
    return parts.join(' ');
  }
  ...
  async function getLmStudioConnectionStatus({ force = false } = {}) {
    ...
    try {
      ...
      value = {
        ...
        runtimePreferredChatModel: runtimePreferredChatModel || null,
        runtimePreferredToolModel: runtimePreferredToolModel || null,
        ...
        hint: resolvedChatModel ? '' : (strictChatHint || `${localRuntimeLabel} is reachable, but no usable chat model is currently loaded.`),
        error: '',
      ...
      const rawMsg = String(error?.message || `${localRuntimeLabel} is unreachable.`);
      const publicFailure = toPublicProviderError(error, {
        provider: localLlmBackend,
        operation: 'models-status',
      });
      let detail = error?.name === 'AbortError'
        ? `${localRuntimeLabel} models request timed out.`
        : isProviderError(error)
          ? publicFailure.message
          : `${localRuntimeLabel} is unavailable.`;
      if (localLlmBackend === 'lm_studio' && settings?.enableLocalService === false) {
        ...
        if (code === 'ECONNREFUSED' || /\bfetch failed\b/i.test(rawMsg)) {
          detail = `Cannot reach the configured endpoint. Start ${localRuntimeLabel}'s OpenAI-compatible server and load a chat model, or check the configured local endpoint.`;
        }
      }
      value = {
        ok: false,
  ...
  function isMissingLmStudioModelError(error) {
    if (error?.reasonCode === 'missing_model') return true;
  ...
  function setRuntimePreferredToolModel(model = '') {
    runtimePreferredToolModel = String(model || '').trim();
    resetLmStudioStatusCache();
    return runtimePreferredToolModel;
  }

  function getRuntimePreferredToolModel() {
    return runtimePreferredToolModel;
  }

  function setRuntimeModelFallbackDisabled(disabled = false) {
    modelFallbackDisabled = disabled === true
      || ['1', 'true', 'yes', 'on'].includes(String(disabled || '').trim().toLowerCase());
    resetLmStudioStatusCache();
    return modelFallbackDisabled;
  }

  function getRuntimeModelFallbackDisabled() {
    return modelFallbackDisabled;
  }
  ...
  return {
    ...
    getRuntimePreferredToolModel,
    setRuntimePreferredToolModel,
    getRuntimeModelFallbackDisabled,
    setRuntimeModelFallbackDisabled,
    resetLmStudioStatusCache,
