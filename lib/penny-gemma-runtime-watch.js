...
const GEMMA_RUNTIME_WATCH_LIMITS = Object.freeze([
  ...
  'Penny omits reasoning controls for normal companion chat; provider-effective state remains unknown without response evidence.',
...
function buildGemmaRuntimeWatchArtifact({
  ...
  const normalizedWatchItems = {
    ...
    thinkingControls: {
      ...
      requestPolicyForCompanionChat: 'not-requested',
  ...
  const normalizedReasoningContract = normalizeReasoningContract(
    reasoningContract || buildStatusReasoningContract({
      capabilityState: thinkingInput.exposed === true ? 'supported' : 'unknown',
      capabilitySource: thinkingInput.exposed === true
        ? 'explicit-control-exposed'
        : 'status-does-not-advertise-reasoning',
    }),
  );
  ...
  return {
    ...
    defaultsUnchanged: {
      ...
      companionReasoningRequestPolicy: 'not-requested',
      providerEffectiveReasoningState: 'unknown',
