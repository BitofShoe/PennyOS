function createDirectToolAssistApi({
  ...
  async function executeDirectWebInspectIntent({ intent, onToolEvent }) {
    ...
    try {
      ...
      return {
        toolsUsed,
        results,
        fallbackText: `i tried to search the web for "${query}", but the search service failed. try again in a moment.\n[MOOD:annoyed]`,
      };
    }
    ...
    try {
      ...
      pageResult = {
        ...
        data: { error: 'The page reader could not complete this request.', url: top.url },
      };
    }
  ...
  async function runDirectToolAssist({ userText, messages, memories, intent, file = null, onToolEvent, abortSignal, laneRuntime = null }) {
    if (intent?.kind === 'deterministic_reply') {
      return {
