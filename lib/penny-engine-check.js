...
function readNpmVersion({
  ...
  const fromUserAgent = npmVersionFromUserAgent(env.npm_config_user_agent || '');
  if (fromUserAgent) return fromUserAgent;

  const npmBin = platform === 'win32' ? 'npm.cmd' : 'npm';
  try {
    return String(execFileSync(npmBin, ['--version'], { encoding: 'utf8' })).trim();
  } catch {
    if (platform !== 'win32') return '';
  }

  try {
    return String(execFileSync(comSpec || 'cmd.exe', ['/d', '/s', '/c', 'npm.cmd --version'], { encoding: 'utf8' })).trim();
  } catch {
    return '';
  }
}
