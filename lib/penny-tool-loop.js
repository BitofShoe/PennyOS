...
function createLmStudioToolLoopApi({
  ...
  function beginToolReasoningReceipt(laneRuntime, source) {
    recordReasoningRequest(laneRuntime, {
      state: 'not-requested',
      source: `${source}-request-payload`,
      control: 'omitted',
    });
  }

  function observeToolReasoningReceipt(laneRuntime, parsed, source) {
    ...
    const alreadyObserved = laneRuntime.reasoningContract?.observed?.state === 'reasoning-observed';
    if (alreadyObserved && !(reasoningChars > 0 || reasoningTokens > 0)) return;
    recordReasoningObservation(laneRuntime, {
      responseCompleted: true,
      source: `${source}-response`,
      signal: reasoningChars > 0 ? 'reasoning-content' : (reasoningTokens > 0 ? 'reasoning-tokens' : 'none'),
      reasoningChars,
  ...
  async function attemptWriteRequiredRescue({
    ...
    try {
      ...
      observeToolReasoningReceipt(laneRuntime, parsed, 'write-rescue');
    } catch {
      updateWriteRescueDebug(debugState, {
  ...
  async function runLmStudioToolContextAnswer({ userText, messages, memories, toolName, toolData, abortSignal, laneRuntime }) {
    return withLmStudioLaneModel('tool', async (model) => {
      beginToolReasoningReceipt(laneRuntime, 'direct-tool-assist');
      ...
      try {
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
          throw providerError('provider_upstream_error', 'direct-tool-assist', {
        ...
        try {
          ...
          observeToolReasoningReceipt(laneRuntime, parsed, 'direct-tool-assist');
        } catch {
          throw providerError('provider_invalid_response', 'direct-tool-assist');
        }
        const message = parsed?.choices?.[0]?.message;
        const text = textFromChatMessage(message);
        if (!text) throw providerError('provider_no_visible_text', 'direct-tool-assist');
        clearLmStudioThread(memories);
        return text.trim();
      } catch (error) {
        if (error?.name === 'AbortError') throw providerError('provider_timeout', 'direct-tool-assist');
        throw error;
      } finally {
        clearTimeout(timer);
      }
  ...
  async function draftOpenEndedWriteText({ userText, messages, memories, path = '', mode = 'direct_open_ended_append', abortSignal, laneRuntime = null }) {
    return withLmStudioLaneModel('tool', async (model) => {
      beginToolReasoningReceipt(laneRuntime, 'open-ended-write');
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), LMSTUDIO_TIMEOUT_MS);
      bindAbortSignal(controller, abortSignal);
      clearLmStudioThread(memories);
      try {
        ...
        async function requestDraftText(messagesForDraft, label = 'open-ended write draft') {
          ...
          const bodyText = response.bodyText;
          if (response.statusCode < 200 || response.statusCode >= 300) {
            throw providerError('provider_upstream_error', 'open-ended-write', {
              upstreamStatus: response.statusCode,
            });
          }
          let parsed;
          try {
            parsed = JSON.parse(bodyText);
            observeToolReasoningReceipt(laneRuntime, parsed, 'open-ended-write');
          } catch {
            throw providerError('provider_invalid_response', 'open-ended-write');
          }
          const message = parsed?.choices?.[0]?.message;
          const text = cleanOpenEndedWriteDraft(textFromChatMessage(message));
          if (!text) throw providerError('provider_no_visible_text', 'open-ended-write');
          return text;
        }
        ...
        if (error?.name === 'AbortError') throw providerError('provider_timeout', 'open-ended-write');
        throw error;
      } finally {
        clearTimeout(timer);
      }
  ...
  async function runLmStudioToolLoop({ userText, messages, memories, onToolEvent, abortSignal, laneRuntime }) {
    return withLmStudioLaneModel('tool', async (model) => {
      beginToolReasoningReceipt(laneRuntime, 'tool-loop');
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), LMSTUDIO_TIMEOUT_MS);
      bindAbortSignal(controller, abortSignal);
      clearLmStudioThread(memories);
      ...
      try {
        for (let step = 0; step < MAX_TOOL_STEPS; step += 1) {
          ...
          const bodyText = response.bodyText;
          if (response.statusCode < 200 || response.statusCode >= 300) {
            throw providerError('provider_upstream_error', 'tool-call', {
              upstreamStatus: response.statusCode,
            });
          }
          let parsed;
          try {
            parsed = JSON.parse(bodyText);
            observeToolReasoningReceipt(laneRuntime, parsed, 'tool-loop');
          } catch {
            throw providerError('provider_invalid_response', 'tool-call');
          }
        ...
        if (error?.name === 'AbortError') throw providerError('provider_timeout', 'tool-call');
        throw error;
      } finally {
        clearTimeout(timer);
      }
  ...
  async function runLmStudioManualToolLoop({ userText, messages, memories, onToolEvent, abortSignal, laneRuntime, fallbackDebug = null }) {
    return withLmStudioLaneModel('tool', async (model) => {
      beginToolReasoningReceipt(laneRuntime, 'manual-tool-loop');
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), LMSTUDIO_TIMEOUT_MS);
      bindAbortSignal(controller, abortSignal);
      clearLmStudioThread(memories);
      ...
      try {
        for (let step = 0; step < MAX_TOOL_STEPS; step += 1) {
          ...
          const bodyText = response.bodyText;
          if (response.statusCode < 200 || response.statusCode >= 300) {
            throw providerError('provider_upstream_error', 'manual-planner', {
              upstreamStatus: response.statusCode,
            });
          }
          let parsed;
          try {
            parsed = JSON.parse(bodyText);
            observeToolReasoningReceipt(laneRuntime, parsed, 'manual-tool-loop');
          } catch {
            throw providerError('provider_invalid_response', 'manual-planner');
          }
          recordLmStudioResponseLimitTelemetry(laneRuntime, extractLmStudioResponseLimitTelemetry({
            parsed,
            payload,
            source: 'manual-tool-loop',
            lane: 'tool',
            step,
          }));
          const message = parsed?.choices?.[0]?.message || {};
        ...
        if (error?.name === 'AbortError') throw providerError('provider_timeout', 'manual-planner');
        throw error;
      } finally {
        clearTimeout(timer);
      }
