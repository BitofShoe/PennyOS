const READINESS_SUMMARY_VERSION = 'penny-local-readiness-summary.v2';
...
function buildLocalReadinessSummary({
  ...
  const compatibilityFallbackActive = chat.fallback || tool.fallback;
  const semanticDegradationActive = semanticMemory.ready !== true && !!semanticMemory.requestedModel;
  const availabilityReady = !requiredLaneMissing && !semanticRequiredMissing;
  const degradationReasons = uniqueStrings([
    ...(requiredLaneMissing ? ['required-lane-unavailable'] : []),
    ...(laneMismatch ? ['lane-model-mismatch'] : []),
    ...(duplicateProblems.length ? ['duplicate-loaded-models'] : []),
    ...(compatibilityFallbackActive ? ['compatibility-fallback'] : []),
    ...(semanticDegradationActive ? ['semantic-degradation'] : []),
    ...(degradedCount > 0 ? ['runtime-artifact-degradation'] : []),
  ], 12);
  const details = [
    chat.message,
    tool.required || requestedToolModel || resolvedToolModel ? tool.message : '',
  ...
  return {
    ...
    availability: {
      ...
      semanticRequiredReady: !semanticRequiredMissing,
    },
    compatibilityFallback: {
      ...
      reasons: uniqueStrings([
        ...(chat.fallback ? ['chat-model-fallback'] : []),
        ...(tool.fallback ? ['tool-model-fallback'] : []),
      ], 8),
    },
    semanticDegradation: {
      active: semanticDegradationActive,
      mode: semanticMemory.mode,
      required: semanticMemory.required,
      reason: trimText(semanticReason, 160),
    },
    degradation: {
      active: degradationReasons.length > 0,
      reasons: degradationReasons,
    },
    legacyFallbackProjection: {
      active: state === READINESS_STATES.FALLBACK
        || state === READINESS_STATES.READY_WITH_OPTIONAL_FALLBACK,
      sources: uniqueStrings([
        ...(compatibilityFallbackActive ? ['compatibility-fallback'] : []),
        ...(semanticDegradationActive ? ['semantic-degradation'] : []),
      ], 8),
      deprecatedStateField: 'state',
    },
