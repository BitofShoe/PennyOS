...
} = require('./penny-memory-export');
const {
  isProviderError,
  toPublicProviderError,
} = require('./penny-provider-errors');
const {
  createStreamEventForwarder,
} = require('./penny-stream-state');

function createRouteHttpError(statusCode = 500, message = 'Penny route failed.') {
...
function buildTurnReadiness({
  ...
  if (requestedMode === 'shadow') {
    return normalizeReadiness({
      ...
      semanticDegradation: {
        active: semanticMemoryReady !== true,
        mode: semanticMemoryMode,
        required: false,
        reason: semanticMemoryReady === true ? '' : 'semantic-memory-not-ready',
      },
      degradation: {
        active: usedFallback === true || semanticMemoryReady !== true,
        reasons: [
          ...(usedFallback === true ? ['shadow-route-fallback'] : []),
          ...(semanticMemoryReady !== true ? ['semantic-degradation'] : []),
        ],
      },
      modelUsage: 'used',
  ...
  if (modelUsed === false) {
    return normalizeReadiness({
      ...
      degradation: {
        active: usedFallback === true || semanticMemoryReady !== true,
        reasons: [
          ...(usedFallback === true ? ['route-fallback'] : []),
          ...(semanticMemoryReady !== true ? ['semantic-degradation'] : []),
        ],
      },
  ...
  const compatibilityFallbackActive = laneFallback === true;
  const semanticDegradationActive = semanticMemoryReady !== true;
  const fallbackActive = usedFallback === true || compatibilityFallbackActive || semanticDegradationActive;
  ...
  return normalizeReadiness({
    ...
    fallbackActive,
    availability: {
      providerReachable: hasResolvedModel,
      chatReady: chatModelReady,
      toolReady: toolModelReady,
      embeddingReady: semanticMemoryReady === true,
      requiredReady: hasResolvedModel,
    },
    compatibilityFallback: {
      active: compatibilityFallbackActive,
      chat: compatibilityFallbackActive && selectedLane === 'chat',
      tool: compatibilityFallbackActive && selectedLane === 'tool',
      reasons: compatibilityFallbackActive ? ['lane-model-fallback'] : [],
    },
    semanticDegradation: {
      active: semanticDegradationActive,
      mode: semanticMemoryMode,
      required: false,
      reason: semanticDegradationActive ? 'semantic-memory-not-ready' : '',
    },
    degradation: {
      active: fallbackActive || !hasResolvedModel,
      reasons: [
        ...(!hasResolvedModel ? ['model-unavailable'] : []),
        ...(usedFallback === true ? ['route-fallback'] : []),
        ...(compatibilityFallbackActive ? ['compatibility-fallback'] : []),
        ...(semanticDegradationActive ? ['semantic-degradation'] : []),
      ],
    },
    modelUsage: normalizedModelUsage,
    warmState,
...
function createPennyRouteHandlers(deps = {}) {
  const {
    ...
    setRuntimePreferredToolModel = null,
    getRuntimePreferredToolModel = null,
    setRuntimePreferredEmbedModel = null,
    getRuntimePreferredEmbedModel = null,
    setRuntimeModelFallbackDisabled = null,
    getRuntimeModelFallbackDisabled = null,
    getProviderStatus = null,
    connectOpenAiProvider = null,
    resetLocalProvider = null,
    getWebSettings = null,
    saveWebSettings = null,
  ...
  function readImageAttachments(payload = {}) {
    const rawImages = Array.isArray(payload?.images)
      ? payload.images
      : (payload?.image ? [payload.image] : []);
    if (typeof sanitizeImageAttachments === 'function') return sanitizeImageAttachments(rawImages);
    if (rawImages.length > 4) {
      throw createRouteHttpError(413, 'Too many images. Keep each turn to 4 images or fewer.');
    }
    return rawImages.map(sanitizeImageDataUrl).filter(Boolean);
  }
  ...
  const {
    ...
    WEB_SEARCH_ENABLED,
    WEB_ANSWER_MODE = 'model',
    PENNY_ENABLE_REVIEW_SIDECARS = false,
  } = constants || {};
  const localLlmBackend = String(LOCAL_LLM_BACKEND || 'lm_studio').trim() || 'lm_studio';
  const localRuntimeLabel = String(LOCAL_RUNTIME_LABEL || 'LM Studio').trim() || 'LM Studio';
  const localEndpointBase = String(LOCAL_ENDPOINT_BASE || LMSTUDIO_BASE || '').trim();

  function publicProviderFailure(error, {
    provider = localLlmBackend,
    operation = 'chat',
  } = {}) {
    return toPublicProviderError(error, { provider, operation });
  }
  ...
  function scrubProviderSecretStrings(value) {
    return String(value || '').replace(/\bsk-[A-Za-z0-9_-]{8,}\b/g, 'sk-...redacted');
  }

  function sanitizeProviderRouteValue(value, seen = new WeakSet()) {
    if (value == null) return value;
    if (typeof value === 'string') return scrubProviderSecretStrings(value);
    if (typeof value !== 'object') return value;
    if (seen.has(value)) return null;
    seen.add(value);
    if (Array.isArray(value)) return value.map((item) => sanitizeProviderRouteValue(item, seen));
    const out = {};
    for (const [key, item] of Object.entries(value)) {
      if (['apiKey', 'PENNY_LMSTUDIO_API_KEY', 'authorization', 'Authorization', 'accessToken', 'token', 'secret'].includes(key)) {
        out[key] = item ? 'redacted' : '';
        continue;
      }
      out[key] = sanitizeProviderRouteValue(item, seen);
    }
    return out;
  }
  ...
  function buildResponseMeta({
    ...
    const meta = {
      ...
      promptTruth,
      toolsUsed,
      toolOutcome: toolOutcome && typeof toolOutcome === 'object'
        ? normalizeToolOutcome(toolOutcome)
        : null,
    };
    if (repair && typeof repair === 'object') meta.repair = repair;
    if (responseLimit && typeof responseLimit === 'object') meta.responseLimit = responseLimit;
    ...
    if (reasoningContract && typeof reasoningContract === 'object') meta.reasoningContract = reasoningContract;
  ...
  function getHealthyStoredMemory(sessionId = 'default') {
    const stored = getStoredMemory(sessionId);
    if (stored.status?.ok !== false) return stored;
    const error = new Error(stored.status.message || 'Penny canonical memory store is unavailable.');
    error.code = stored.status.code || 'memory_store_unavailable';
    error.statusCode = error.code === 'invalid-schema' ? 409 : 500;
    throw error;
  }

  function sendMemoryStoreError(res, error) {
    sendJson(res, error?.statusCode || 500, {
      ok: false,
      code: error?.code || 'memory_store_unavailable',
      error: error?.message || 'Penny canonical memory store is unavailable.',
    });
  }

  async function handleMemoryRoutes({ req, res, url }) {
    if (req.method === 'GET' && url.pathname === '/api/penny/memory/export') {
      try {
        const sessionId = url.searchParams.get('sessionId') || 'default';
        const { memory } = getHealthyStoredMemory(sessionId);
        sendJson(res, 200, { ok: true, export: buildExplicitMemoryExport(memory) });
      } catch (error) {
        sendMemoryStoreError(res, error);
      }
      return true;
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/penny/memory') {
      try {
        ...
        const existing = getHealthyStoredMemory(sessionId).memory;
    ...
    if (req.method === 'GET' && url.pathname === '/api/penny/memory/inspector') {
      try {
        const sessionId = url.searchParams.get('sessionId') || 'default';
        const explicitMemory = getHealthyStoredMemory(sessionId).memory;
    ...
    if (req.method === 'POST' && url.pathname === '/api/penny/memory/review') {
      try {
        ...
        let memory = getHealthyStoredMemory(sessionId).memory;
    ...
    if (req.method === 'POST' && url.pathname === '/api/penny/consolidate') {
      try {
        ...
        const sessionId = payload.sessionId || 'default';
        getHealthyStoredMemory(sessionId);
    ...
    if (req.method === 'POST' && url.pathname === '/api/penny/chat/shadow') {
      try {
        ...
        try {
          ...
          const publicFailure = publicProviderFailure(error, {
            provider: 'openclaw',
            operation: 'legacy-shadow-chat',
          });
          const fallbackText = buildPennyReply({ userText, memories: promptMemories });
          scheduleArchiveConsolidation({
            sessionId,
  ...
  async function handleLmStudioRoutes({ req, res, url }) {
    if (req.method === 'GET' && url.pathname === '/api/penny/lmstudio/status') {
      ...
      sendJson(res, 200, {
        ...lmStudio,
        localLlmBackend: lmStudio.localLlmBackend || localLlmBackend,
        localRuntimeLabel: lmStudio.localRuntimeLabel || localRuntimeLabel,
        localEndpointBase: lmStudio.localEndpointBase || localEndpointBase,
        embedPreferredModel: typeof getRuntimePreferredEmbedModel === 'function'
          ? (getRuntimePreferredEmbedModel() || PENNY_LMSTUDIO_EMBED_MODEL)
          : PENNY_LMSTUDIO_EMBED_MODEL,
        embedBase: LMSTUDIO_EMBED_BASE || LMSTUDIO_BASE,
        semanticMemory,
        readiness,
        performance,
      });
    ...
    if (req.method === 'POST' && url.pathname === '/api/penny/lmstudio/model') {
      try {
        const payload = await parseJsonBody(req);
        const chatModel = String(payload.chatModel ?? payload.model ?? '').trim();
        const toolModel = String(payload.toolModel ?? '').trim();
        const embedModel = String(payload.embedModel ?? '').trim();
        const hasChatModel = Object.prototype.hasOwnProperty.call(payload, 'chatModel')
          || Object.prototype.hasOwnProperty.call(payload, 'model');
        const hasToolModel = Object.prototype.hasOwnProperty.call(payload, 'toolModel');
        const hasEmbedModel = Object.prototype.hasOwnProperty.call(payload, 'embedModel');
        const hasFallbackPolicy = Object.prototype.hasOwnProperty.call(payload, 'disableModelFallback');
        if (hasChatModel) setRuntimePreferredChatModel(chatModel);
        if (hasToolModel) {
          if (typeof setRuntimePreferredToolModel !== 'function') {
            throw createRouteHttpError(501, 'Tool model runtime selection is not available in this build.');
          }
          setRuntimePreferredToolModel(toolModel);
        }
        if (hasEmbedModel) {
          if (typeof setRuntimePreferredEmbedModel !== 'function') {
            throw createRouteHttpError(501, 'Embedding model runtime selection is not available in this build.');
          }
          setRuntimePreferredEmbedModel(embedModel);
        }
        if (hasFallbackPolicy) {
          if (typeof setRuntimeModelFallbackDisabled !== 'function') {
            throw createRouteHttpError(501, 'Model fallback runtime selection is not available in this build.');
          }
          setRuntimeModelFallbackDisabled(payload.disableModelFallback === true);
        }
        const lmStudio = await getLmStudioConnectionStatus({ force: true });
        const semanticMemory = await getSemanticMemoryStatus({ force: true, lmStatus: lmStudio });
        sendJson(res, 200, {
          ok: true,
          runtimePreferredModel: getRuntimePreferredChatModel() || null,
          runtimePreferredChatModel: getRuntimePreferredChatModel() || null,
          runtimePreferredToolModel: typeof getRuntimePreferredToolModel === 'function'
            ? (getRuntimePreferredToolModel() || null)
            : (lmStudio.runtimePreferredToolModel || null),
          runtimePreferredEmbedModel: typeof getRuntimePreferredEmbedModel === 'function'
            ? (getRuntimePreferredEmbedModel() || null)
            : (semanticMemory.configuredModel || null),
          embedPreferredModel: semanticMemory.configuredModel || null,
          semanticMemory,
          ...
          modelFallbackDisabled: typeof getRuntimeModelFallbackDisabled === 'function'
            ? getRuntimeModelFallbackDisabled()
            : lmStudio.modelFallbackDisabled === true,
  ...
  async function handleProviderRoutes({ req, res, url }) {
    if (req.method === 'GET' && url.pathname === '/api/penny/provider/status') {
      if (typeof getProviderStatus !== 'function') {
        sendJson(res, 503, { ok: false, error: 'Provider settings are not available in this build.' });
        return true;
      }
      sendJson(res, 200, sanitizeProviderRouteValue(getProviderStatus()));
      return true;
    }

    if (req.method === 'POST' && url.pathname === '/api/penny/provider/openai/connect') {
      if (typeof connectOpenAiProvider !== 'function') {
        sendJson(res, 503, { ok: false, error: 'OpenAI cloud provider setup is not available in this build.' });
        return true;
      }
      try {
        const payload = await parseJsonBody(req);
        if (payload?.acceptCloudDisclosure !== true) {
          sendJson(res, 400, {
            ok: false,
            error: 'Confirm the cloud privacy disclosure before connecting OpenAI.',
          });
          return true;
        }
        const result = await connectOpenAiProvider(payload);
        sendJson(res, 200, sanitizeProviderRouteValue(result));
      } catch (error) {
        if (isProviderError(error)) {
          ...
          sendJson(res, error?.statusCode || 500, {
            ok: false,
            error: scrubProviderSecretStrings(error.message || 'OpenAI cloud provider setup failed.'),
          });
        }
      }
      return true;
    ...
    if (req.method === 'POST' && url.pathname === '/api/penny/provider/local/reset') {
      if (typeof resetLocalProvider !== 'function') {
        sendJson(res, 503, { ok: false, error: 'Local provider reset is not available in this build.' });
        return true;
      }
      try {
        const payload = await parseJsonBody(req);
        const result = await resetLocalProvider(payload);
        sendJson(res, 200, sanitizeProviderRouteValue(result));
      } catch (error) {
        sendJson(res, error?.statusCode || 500, {
          ok: false,
          error: scrubProviderSecretStrings(error.message || 'Local provider reset failed.'),
        });
      }
      return true;
    }
  ...
  async function handleWebSettingsRoutes({ req, res, url }) {
    if (req.method === 'GET' && url.pathname === '/api/penny/web-settings') {
      if (typeof getWebSettings !== 'function') {
        sendJson(res, 503, { ok: false, error: 'Web settings are not available in this build.' });
        return true;
      }
      sendJson(res, 200, getWebSettings());
      return true;
    }

    if (req.method === 'POST' && url.pathname === '/api/penny/web-settings') {
      if (typeof saveWebSettings !== 'function') {
        sendJson(res, 503, { ok: false, error: 'Web settings are not available in this build.' });
        return true;
      }
      try {
        const payload = await parseJsonBody(req);
        sendJson(res, 200, saveWebSettings(payload));
      } catch (error) {
        sendJson(res, error?.statusCode || 500, { ok: false, error: error.message });
      }
      return true;
  ...
  async function handleChatRoutes({ req, res, url }) {
    ...
    try {
      ...
      const imageAttachments = readImageAttachments(payload);
      const images = imageAttachments.map(attachment => attachment?.dataUrl).filter(Boolean);
      const imagePayload = images.length ? images : null;
      const fileAttachment = sanitizeFileAttachment(payload.file || null);
      const promptUserText = appendAttachmentContext(userText, fileAttachment);
      ...
      const laneSelection = requestedMode === 'local'
        ? selectLocalLane({ userText, image: imagePayload, file: fileAttachment })
      ...
      const runtimeMemoryContext = await buildRuntimeMemoryContext({
        ...
        attachmentType: images.length ? 'image' : (fileAttachment ? 'file' : 'none'),
      });
      ...
      if (wantsStream) {
        ...
        const forwardStreamEvent = createStreamEventForwarder((evt) => {
          ...
          if (evt?.type === 'message.delta') {
            ...
            sendEventStream(res, 'message.delta', {
              content: evt.content || '',
              text: evt.text || '',
            });
          } else if (evt?.type === 'status') {
            sendEventStream(res, 'status', {
              stage: evt.stage || '',
              label: evt.label || '',
            });
          } else if (evt?.type === 'tool') {
            sendEventStream(res, 'tool', evt);
          } else if (evt?.type === 'stream.reset') {
            sendEventStream(res, 'stream.reset', {
              reason: evt.reason || 'transport-retry',
            });
          }
        });

        try {
          forwardStreamEvent({ type: 'status', stage: 'accepted', label: 'link open' });
          if (requestedMode === 'local') {
            ...
            responseLimit = result.responseLimit && typeof result.responseLimit === 'object' ? result.responseLimit : null;
            ...
            reasoningContract = result.reasoningContract && typeof result.reasoningContract === 'object'
          ...
          text = retagAssistantReply(text, '', {
            balanceMood: true,
            previousMood: sessionState.lastMood,
          });
          if (requestedMode === 'local' && images.length && !clientDisconnect.isClosed()) {
            forwardStreamEvent({ type: 'status', stage: 'image.reply.ready', label: 'replying' });
            forwardStreamEvent({ type: 'message.delta', content: text, text });
          }
          ...
          const publicFailure = publicProviderFailure(error, {
            provider: requestedMode === 'local' ? localLlmBackend : 'openclaw',
            operation: requestedMode === 'local' ? 'chat-stream' : 'shadow-stream',
          });
          if (!clientDisconnect.isClosed()) {
            sendEventStream(res, 'error', {
              error: requestedMode === 'local' ? `Local ${localRuntimeLabel} brain failed.` : 'Penny chat route failed.',
              detail: publicFailure.message,
              code: publicFailure.code,
              retryable: publicFailure.retryable,
              providerFailure: publicFailure,
              meta: {
                requestedMode,
                ...
                laneFallback,
                toolsUsed,
                ...(shadowError ? { shadowError: publicFailure.message } : {}),
              },
            });
            res.end();
          }
      ...
      try {
        if (requestedMode === 'local') {
          try {
            ...
            sendJson(res, publicFailure.statusCode, {
              error: `Local ${localRuntimeLabel} brain failed.`,
              detail: publicFailure.message,
              code: publicFailure.code,
              retryable: publicFailure.retryable,
              providerFailure: publicFailure,
      ...
      sendJson(res, statusCode >= 400 && statusCode < 500 ? statusCode : 500, {
        error: 'Penny chat route failed.',
        detail: statusCode >= 400 && statusCode < 500
          ? String(error?.message || 'The request was invalid.')
  ...
  async function handleStatusRoutes({ req, res, url }) {
    ...
    sendJson(res, 200, {
      ...
      webAnswerMode: WEB_ANSWER_MODE,
      lmStudioBase: LMSTUDIO_BASE,
      lmStudioEmbedBase: LMSTUDIO_EMBED_BASE || LMSTUDIO_BASE,
  ...
  async function handleSidecarRoutes({ req, res, url }) {
    const sidecarRoutes = {
      '/api/penny/sidecars/search': 'search',
      '/api/penny/sidecars/docs': 'docs',
      '/api/penny/sidecars/rag': 'docs',
      '/api/penny/sidecars/audio': 'audio',
      '/api/penny/sidecars/tts': 'audio',
    };
    const kind = sidecarRoutes[url.pathname];
    if (!kind) return false;
    if (req.method !== 'POST') {
      sendJson(res, 405, { ok: false, error: 'Unsupported sidecar workflow route.' });
      return true;
    }
    if (PENNY_ENABLE_REVIEW_SIDECARS !== true) {
      sendJson(res, 410, {
        ok: false,
        error: 'Review sidecar workflows are disabled in this build. Set PENNY_ENABLE_REVIEW_SIDECARS=1 for source/dev harness work.',
      });
      return true;
    }
    if (typeof runSidecarWorkflow !== 'function') {
      sendJson(res, 503, { ok: false, error: 'Sidecar workflows are not available.' });
      return true;
    }
    try {
      const payload = await parseJsonBody(req);
      const workflow = await runSidecarWorkflow(kind, payload);
      const blocked = workflow?.ok === false;
      const failureReason = String(workflow?.failure?.reason || '');
      const statusCode = blocked && failureReason.endsWith('permission_required') ? 403 : 200;
      ...
      sendJson(res, 500, { ok: false, error: String(error?.message || error).trim() });
      return true;
    }
  }

  async function handleRuntimeVoiceRoutes({ req, res, url }) {
    if (!url.pathname.startsWith('/api/penny/voice/')) return false;
    if (!runtimeVoice || typeof runtimeVoice !== 'object') {
      sendJson(res, 503, { ok: false, error: 'Runtime voice is not available.' });
      return true;
    }
    try {
      if (req.method === 'GET' && url.pathname === '/api/penny/voice/status') {
        if (typeof runtimeVoice.getStatus !== 'function') {
          sendJson(res, 503, { ok: false, error: 'Runtime voice status is not available.' });
          return true;
        }
        sendJson(res, 200, await runtimeVoice.getStatus());
        return true;
      }
      if (req.method === 'POST' && url.pathname === '/api/penny/voice/config') {
        if (typeof runtimeVoice.setConfig !== 'function') {
          sendJson(res, 503, { ok: false, error: 'Runtime voice config is not available.' });
          return true;
        }
        const payload = await parseJsonBody(req);
        const configResult = await runtimeVoice.setConfig(payload);
        const status = typeof runtimeVoice.getStatus === 'function'
          ? await runtimeVoice.getStatus()
          : null;
        sendJson(res, 200, { ok: true, ...configResult, status });
        return true;
      }
      if (req.method === 'POST' && url.pathname === '/api/penny/voice/speech') {
        if (typeof runtimeVoice.synthesizeSpeech !== 'function') {
          sendJson(res, 503, { ok: false, error: 'Runtime voice speech is not available.' });
          return true;
        }
        const payload = await parseJsonBody(req);
        const result = await runtimeVoice.synthesizeSpeech({
          text: payload?.text,
          voice: payload?.voice,
          gain: payload?.gain,
          speed: payload?.speed,
        ...
        res.writeHead(200, {
          ...
          'X-Penny-Voice-Provider': result?.provider || 'speaches',
        });
        res.end(audioBuffer);
        return true;
      }
      sendJson(res, 405, { ok: false, error: 'Unsupported runtime voice route.' });
      return true;
    } catch (error) {
      const rawStatusCode = Number(error?.statusCode || 500);
      const statusCode = Number.isInteger(rawStatusCode) && rawStatusCode >= 400 && rawStatusCode <= 599
        ? rawStatusCode
        : 500;
      sendJson(res, statusCode, {
        ok: false,
        code: error?.code || 'runtime_voice_error',
  ...
  async function handleApiRoute(context) {
    ...
    if (await handleProviderRoutes(context)) return true;
    if (await handleWebSettingsRoutes(context)) return true;
    if (await handleLmStudioRoutes(context)) return true;
    if (await handleWorkspaceWriteRoutes(context)) return true;
    if (await handleRuntimeVoiceRoutes(context)) return true;
    if (await handleSidecarRoutes(context)) return true;
