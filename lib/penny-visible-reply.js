...
function createVisibleReplyApi({
  ...
  function cleanDraftCandidate(text = '') {
    ...
    // Normalize repeated horizontal spacing without consuming intentional
    // paragraph breaks. `\s` also matches newlines, so using `\s{2,}` here
    // turned every blank line into a space when the streamed reply was
    // finalized and made the completed transcript a wall of text.
  ...
  function textFromChatMessage(msg) {
    ...
    if (ALLOW_RAW_REASONING_FALLBACK && (!out || looksOnlyLikeCoT(out))) {
  ...
  function isMissingLmStudioThreadError(error) {
    if (error?.reasonCode === 'missing_thread') return true;
