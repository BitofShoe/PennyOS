...
- [scripts/export-penny-semantic-claims.js](./scripts/export-penny-semantic-claims.js)
Local read-only `penny-semantic-export.v1` JSON artifact writer for normalized semantic claims, memory links, domain registry entries, predicate registry entries, and local semantic IDs. It can use the built-in fixture or a local JSON input file; it does not read live memory by default, call LM Studio, publish Linked Data, dereference IDs, or add RDF/SPARQL/triplestore dependencies.
...
- [lib/penny-semantic-export.js](./lib/penny-semantic-export.js)
