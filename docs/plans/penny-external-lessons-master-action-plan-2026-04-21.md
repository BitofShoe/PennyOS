...
- Gemma runtime watch landed as fixture/status artifact coverage only. `penny-gemma-runtime-watch.v1`, status/preflight data, and `npm run eval:runtime-fit:gemma-watch` record watch items such as vision-budget exposure, omitted reasoning request controls, provider-effective state remaining unknown without response evidence, prompt-cache/RAM risk, loaded-model identity, current-turn image policy, and chat sampling; they do not run live chat generation or change LM Studio defaults.
