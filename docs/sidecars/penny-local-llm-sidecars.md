...
Source/dev workflow docs: `docs/sidecars/penny-sidecar-productized-workflows.md`. That document covers the env-gated API harness path. The consumer Settings UI no longer exposes these sidecars, and the section trial commands below remain the lower-level harness and live-smoke surface.
